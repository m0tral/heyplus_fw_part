...
#ifdef APOLLO_HARDWARE_BLOCK_FLUSH
	#include "ry_utils.h"
	#define LOG_DEBUG_I
	#define LOG_DEBUG_N
#endif
...
gdispImageError gdispGImageDraw_GIF(GDisplay *g, gdispImage *img, coord_t x, coord_t y, coord_t cx, coord_t cy, coord_t sx, coord_t sy) {
	...
	if (priv->frame.flags & GIFL_INTERLACE) {
...
#ifndef APOLLO_HARDWARE_BLOCK_FLUSH	
	else {
		// Every row in sequence
		...
		uint32_t block_num = (72 * 128) / GDISP_IMAGE_GIF_BLIT_BUFFER_SIZE;
		#define BLOCK_STEP_RATE (GDISP_IMAGE_GIF_BLIT_BUFFER_SIZE / priv->frame.width)
		#define gif_start_x (x+mx-sx-gcnt+1)
        #define gif_start_y (y+my-sy)
		#define gif_count_x (priv->frame.width)
		#define gif_count_y (gcnt / priv->frame.width)
		mx = my = gcnt = 0;
		LOG_DEBUG_I("new picture 0: mx:%d, my:%d, pwidth:%d, pheight:%d, block_num:%d, start_y:%d, count_x:%d, count_y:%d\r\n\r\n", mx, my, priv->frame.width, priv->frame.height, block_num, gif_start_y, gif_count_x, gif_count_y);
		LOG_DEBUG_N("new picture: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
		for(my=0; my < priv->frame.height; my += BLOCK_STEP_RATE) {
			LOG_DEBUG_N("new line:mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
			for(gcnt=0, mx=0; mx < GDISP_IMAGE_GIF_BLIT_BUFFER_SIZE; mx++, q++, cnt--) {
				LOG_DEBUG_N("loop 0: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", 	mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
				
				if (!cnt) {
					if (!(cnt = getBytesGif(img))) {
						// Sometimes the image EOF is a bit early - treat the rest as transparent
						if (decode->code_last != decode->code_eof){
							LOG_DEBUG_I("error break: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
							goto baddatacleanup;
						}
						mx++;
						LOG_DEBUG_I("read break: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
						break;
					}
					q = decode->buf;
					LOG_DEBUG_I("my-%d, get read: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", my, mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
				}
				LOG_DEBUG_N("loop 1: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d, sy:%d, fy:%d, sx:%d, fx:%d\r\n", 	mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y, sy, fy, sx, fx);
				if (my >= sy && my < fy && mx >= sx && mx < GDISP_IMAGE_GIF_BLIT_BUFFER_SIZE) {
					LOG_DEBUG_N("loop 1.1: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", 	mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
					col = *q;
					LOG_DEBUG_N("loop 1.2: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", 	mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
					if ((priv->frame.flags & GIFL_TRANSPARENT) && col == priv->frame.paltrans) {
						LOG_DEBUG_I("dump break: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", 	mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
						// We have a transparent pixel - dump the buffer to the display
						switch(gcnt) {
							case 0:																							break;
							case 1:		gdispGDrawPixel(g, x+mx-sx-gcnt, y+my-sy, priv->buf[0]); gcnt = 0;					break;
							default:	gdispGBlitArea(g, gif_start_x, gif_start_y, gif_count_x, gif_count_y, 0, 0, gcnt, priv->buf); gcnt = 0;	
										// We have run out of buffer - dump it to the display
										LOG_DEBUG_I("flush 0: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
							break;
						}
						// We have run out of buffer - dump it to the display
						LOG_DEBUG_I("continue 1: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
						continue;
					}
					LOG_DEBUG_N("loop 2: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
					priv->buf[gcnt++] = decode->palette[col];
					//LOG_DEBUG_I("add buffer: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", 	mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
					if (gcnt >= GDISP_IMAGE_GIF_BLIT_BUFFER_SIZE) {
						// We have run out of buffer - dump it to the display
						LOG_DEBUG_I("flush 1: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
						gdispGBlitArea(g, gif_start_x, gif_start_y, gif_count_x, gif_count_y, 0, 0, gcnt, priv->buf);
						gcnt = 0;
					}
					if((my == 62) && (gcnt == 0)){
						LOG_DEBUG_I("continue 2: mx:%d, my:%d, cnt:%d, gcnt:%d, col:%d, start_y:%d, q:%d, count_y:%d\r\n", mx, my, cnt, gcnt, col, gif_start_y, q, gif_count_y);
					}
					continue;
				}
				LOG_DEBUG_I("finish visible area: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d, sy:%d, fy:%d, sx:%d, fx:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y, sy, fy, sx, fx);				
				// We have finished the visible area - dump the buffer to the display
				switch(gcnt) {
					case 0:																							break;
					case 1:		gdispGDrawPixel(g, x+mx-sx-gcnt, y+my-sy, priv->buf[0]); gcnt = 0;					break;
					default:	gdispGBlitArea(g, gif_start_x, gif_start_y, gif_count_x, gif_count_y, 0, 0, gcnt, priv->buf); gcnt = 0;	
								LOG_DEBUG_I("flush 2: mx1:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
					break;
				}
				LOG_DEBUG_I("line loop end: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d, sy:%d, fy:%d, sx:%d, fx:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y, sy, fy, sx, fx);
			}	//end of multi-line 一行写完，准备移动到下一组y
			// We have finished the line - dump the buffer to the display
			LOG_DEBUG_N("finished the line: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d, sy:%d, fy:%d, sx:%d, fx:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y, sy, fy, sx, fx);
			//gif img size changed, and make a break read and dump operation, must manual change x_start to zero
			#define GID_MANUAL_START_X		0
			switch(gcnt) {
				case 0:		LOG_DEBUG_N("break line: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d, sy:%d, fy:%d, sx:%d, fx:%d\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y, sy, fy, sx, fx);																			break;
				case 1:		gdispGDrawPixel(g, x+mx-sx-gcnt, y+my-sy, priv->buf[0]);					break;
				default:	gdispGBlitArea(g, GID_MANUAL_START_X, gif_start_y, gif_count_x, gif_count_y - 1, 0, 0, gcnt, priv->buf);	gcnt = 0;
							LOG_DEBUG_I("flush: mx3:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n", 	mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
							break;
			}
			LOG_DEBUG_N("close line: mx:%d, my:%d, pwidth:%d, pheight:%d, block_num:%d, start_y:%d, count_x:%d, count_y:%d\r\n\r\n", mx, my, priv->frame.width, priv->frame.height, block_num, gif_start_y, gif_count_x, gif_count_y);
			//循环执行到此处，移动y坐标			
		}		
		//一幅图全部绘出，循环结束
		LOG_DEBUG_N("end picture: mx:%d, my:%d, cnt:%d, gcnt:%d, start_x:%d, start_y:%d, count_x:%d, count_y:%d\r\n\r\n", mx, my, cnt, gcnt, gif_start_x, gif_start_y, gif_count_x, gif_count_y);
	}
#endif			
