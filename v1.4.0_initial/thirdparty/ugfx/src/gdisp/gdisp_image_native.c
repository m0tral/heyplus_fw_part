 ...
 */
#define BLIT_BUFFER_SIZE_NATIVE		(GDISP_SCREEN_WIDTH * 3 * 20)	//native image is 3 byte, but sizeof_color is 4
...
void gdispImageClose_NATIVE(gdispImage *img) {
	...
	if (priv) {
		if (priv->frame0cache)
			gdispImageFree(img, (void *)priv->frame0cache, img->width * img->height * sizeof_color);
...
gdispImageError gdispImageCache_NATIVE(gdispImage *img) {
	...
	len = img->width * img->height * sizeof_color;
...
gdispImageError gdispGImageDraw_NATIVE(GDisplay *g, gdispImage *img, coord_t x, coord_t y, coord_t cx, coord_t cy, coord_t sx, coord_t sy) {
	...
	pos = FRAME0POS_NATIVE + (img->width * sy + sx) * sizeof_color;
	...
	for(;cy;cy--, y++) {
		...
		for(mx = x, mcx = cx; mcx > 0; mcx -= len, mx += len) {
			...
			int length = mcx;
			length = BLIT_BUFFER_SIZE_NATIVE*sizeof_color;
			
			// Read the data
			len = gfileRead(img->f,
						priv->buf,
						mcx > BLIT_BUFFER_SIZE_NATIVE ? (BLIT_BUFFER_SIZE_NATIVE*sizeof_color) : (mcx * sizeof_color))
					/ sizeof_color;
			if (!len)
				return GDISP_IMAGE_ERR_BADDATA;
