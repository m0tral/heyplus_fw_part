...
static coord_t getPixels(gdispImage *img, coord_t x) {
	...
	case 24:
		{
		uint8_t		b[3 * GDISP_IMAGE_BMP_BLIT_BUFFER_SIZE];
#ifdef APOLLO_HARDWARE_BLOCK_FLUSH			
			while(len < GDISP_IMAGE_BMP_BLIT_BUFFER_SIZE) {
#else				
			while(x < img->width && len < sizeof(b)) {
#endif				
				#define min(a,b)            (((a) < (b)) ? (a) : (b))
				uint32_t len_to_read = min(img->width * 3, sizeof(b));
				if (gfileRead(img->f, &b, len_to_read) != len_to_read)
					return 0;
				uint32_t i;
				for (i = 0; i < len_to_read / 3; i ++){
					*pc++ = RGB2COLOR(b[i * 3 + 2], b[i * 3 + 1], b[i * 3]);
					x++;
					len++;
				}
			}
			...
			if (x >= img->width) {
				...
				if (0 && (x & 3) && gfileRead(img->f, &b, x & 3) != (x & 3))
					return 0;
			}
		}
		return len;
...
gdispImageError gdispGImageDraw_BMP(GDisplay *g, gdispImage *img, coord_t x, coord_t y, coord_t cx, coord_t cy, coord_t sx, coord_t sy) {
	...
	uint16_t count_x;
	uint16_t count_y;
	coord_t image_start_x; 
	coord_t image_start_y; 	
	uint16_t position; 
	...
	if (1){
		int length_to_read;
		pos = priv->frame0pos + (img->width * sy + sx) * sizeof_color;
		...
		for(; cy; cy--, y++) {
			...
			for(mx = x, mcx = cx; mcx > 0; mcx -= len, mx += len) {
				length_to_read = mcx > GDISP_IMAGE_BMP_BLIT_BUFFER_SIZE ? (GDISP_IMAGE_BMP_BLIT_BUFFER_SIZE) : mcx;
				length_to_read = length_to_read * sizeof_color;

				// Read the data
				len = gfileRead(img->f, priv->buf, length_to_read) / sizeof_color;

				// Make sure we have read a multiple of 4 bytes for the line
				if (length_to_read & 3){
					gfileRead(img->f, priv->buf, length_to_read & 3);
				}	
				if (!len)
					return GDISP_IMAGE_ERR_BADDATA;
			...
			pos += (len) * sizeof_color;
		}
	}
#else
	last_pos = 0;
	if (TRUE || (priv->bmpflags & BMP_TOP_TO_BOTTOM)){
		for(my = 0; my < img->height; my++) {
			...
			while(mx < img->width) {
				...
				if (my >= sy && my < sy+cy && mx < sx+cx && mx+pos >= sx) 
                {
					...
					count_x = len;
					count_y = 1;
					LOG_DEBUG_I("new count: start_x: %d, start_y: %d, count_x: %d, count_y: %d,\r\n", start_x, start_y, count_x, count_y);
					...
					LOG_DEBUG_I("skip: start_x: %d, start_y: %d, count_x: %d, count_y: %d,\r\n", start_x, start_y, count_x, count_y);
				}             
				mx += pos;
				last_pos = pos;
			}
...
#ifdef APOLLO_HARDWARE_BLOCK_FLUSH
        uint32_t block_num = (img->height * img->width) / GDISP_IMAGE_BMP_BLIT_BUFFER_SIZE;
		for(my = 0; my <= block_num - 1; my++) {
			mx = 0;
			while(mx < GDISP_IMAGE_BMP_BLIT_BUFFER_SIZE) {
				if (!(pos = getPixels(img, mx)))
					return GDISP_IMAGE_ERR_BADDATA;
                start_x = x + (last_pos % img->width);
				#define RATA_BLOCK (GDISP_IMAGE_BMP_BLIT_BUFFER_SIZE / img->width)
				start_y = y + my * RATA_BLOCK - sy;
				count_x = img->width;
				count_y = pos / img->width;
				end_y = start_y + count_y;

				if (start_y <= 128)
			    	count_y = (end_y < 128) ? count_y: (count_y - (end_y - 128));
				else
					break;
				...
				LOG_DEBUG_I("start_x: %d, start_y: %d, count_x: %d, count_y: %d,\r\n", start_x, start_y, count_x, count_y);
				if (count_y)
					gdispGBlitArea(g, start_x, start_y, count_x, count_y, image_start_x, image_start_y, position, priv->buf);
				mx += pos;
				last_pos = pos;
			}
		}
	...
	LOG_DEBUG_I("one image end. start_x: %d, start_y: %d, count_x: %d, count_y: %d,\r\n\r\n", start_x, start_y, count_x, count_y);
	return GDISP_IMAGE_ERR_OK;
}
