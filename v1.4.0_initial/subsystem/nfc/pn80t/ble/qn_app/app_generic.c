...
extern void unsubscribeCompanionForSeEvents(void);
...
void app_eaci_set_dev_name_cmd(uint8_t val_len, uint8_t *dev_name)
{
  ...
  int      i;
    ...
    {
        phOsalNfc_Log("Insufficient memory available\r\n");
    }
}
  ...
  uint8_t i;
    ...
    case EACI_MSG_EVT_ADV:                         // advertising
        if (param_len == 1)
        {
			if (param[0] == 1)
			{
				phOsalNfc_LogBle("Advertising started.\r\n");
			}
			else
				phOsalNfc_LogBle("Advertising stopped.\r\n");
        }
        break;
    ...
    case EACI_MSG_EVT_INQ_RESULT:                  // Scan result
        if (param_len >= 7)
        {
            ...
            phOsalNfc_Log("%d. %c %02X%02X:%02X:%02X%02X%02X ", app_env.inq_id,
    ...
    case EACI_MSG_EVT_INQ_CMP:                     // Scan completed
        if (param_len == 1)
        {
            phOsalNfc_Log("Total %d devices found.\r\n", param[0]);
        }
        break;
    ...
    case EACI_MSG_EVT_CONN:
        if (param_len == 7)
        {
			...
			phOsalNfc_LogCrit("BLE Connected\n");

			phOsalNfc_Log("Connection with %02X:%02X:%02X:%02X:%02X:%02X result is 0x%x.\r\n",
                    peer_addr.addr[5],
                    peer_addr.addr[4],
                    peer_addr.addr[3],
                    peer_addr.addr[2],
                    ...
                    peer_addr.addr[0],
                    param[0]);
            /* Wait for connection to set right */
            phOsalNfc_Delay(500);
#ifndef NFC_UNIT_TEST
            /* Notify the wearable device's status to the connected companion */
            notifyDeviceStatus(&DeviceStatus);
#endif
        }
        break;
    ...
    case EACI_MSG_EVT_DISC:
        ...
        if (param_len == 7)
        {
            ...
            GeneralState &= ~BLE_CONNECTED_MASK;
			phOsalNfc_LogCrit("BLE DisConnected\n");

            phOsalNfc_Log("Disconnect with %02X%02X:%02X:%02X%02X%02X reason is 0x%x.\r\n",
                    peer_addr.addr[5],
                    peer_addr.addr[4],
                    peer_addr.addr[3],
                    peer_addr.addr[2],
                    ...
                    peer_addr.addr[0],
                    param[0]);
            // release cfm_semaphore on disconnection.
/*
#ifndef NFC_UNIT_TEST
            xSemaphoreGive(cfm_semaphore);
#endif*/

            cleanupOnBleInterruption();
           /* xSemaphoreTake(ble_nfc_semaphore, BLE_CFM_TIMEOUT/portTICK_PERIOD_MS );*/
        }

        //once companion device is disconnected unsubscribe it for Se events
        unsubscribeCompanionForSeEvents();

        
#if (GDI_GUP_ENABLE == FALSE)
		//Start advertising command
		BLE_wakeup();                              // Wake up 902x
		app_eaci_cmd_adv(1, ADV_INTV_MIN, ADV_INTV_MAX);       // Just try advertising
		BLEgotoSleep();
#endif  /* GDI_GUP_ENABLE == TRUE */
    ...
    case EACI_MSG_EVT_BOND:                        // Bond completed
        if (param_len == 8)
        {
            ...
            phOsalNfc_Log("Bond complete with %02X%02X:%02X:%02X%02X%02X, bonded: %01X, status: 0x%02X.\r\n",
                        peer_addr.addr[5],
                        peer_addr.addr[4],
                        peer_addr.addr[3],
                        peer_addr.addr[2],
    ...
    case EACI_MSG_EVT_CEN_UPDATE_PARAM:            // Update Param
        ...
        {
            if (param[0] == 0)
                phOsalNfc_Log("Master update parameter complete.\r\n");
            else
                phOsalNfc_Log("Master update parameter failed.\r\n");
        }
        break;
    ...
    case EACI_MSG_EVT_SMP_SEC:
        if (param_len == 1)
        {
            // Update Param
            if (param[0] == 0)
                phOsalNfc_Log("SMPC Security complete.\r\n");
            else
                phOsalNfc_Log("SMPC Security failed.\r\n");
        }
        break;
    ...
    case EACI_MSG_EVT_SET_DEVNAME:
        if (param_len == 1)
        {
            // Update Param
            if (param[0] == 0)
                phOsalNfc_Log("Set Device Name complete.\r\n");
            else
                phOsalNfc_Log("Set Device Name failed.\r\n");
        }
        break;
    ...
    case EACI_MSG_EVT_UPDATE_PARAM:
        if (param_len == 1)
        {
            // Update Param
            if (param[0] == 0)
                phOsalNfc_Log("Slave update success.\r\n");
            else
                phOsalNfc_Log("Slave update failed.\r\n");
        }
        break;
