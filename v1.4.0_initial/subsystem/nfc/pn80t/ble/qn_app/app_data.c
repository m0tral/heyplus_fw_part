    ...
    switch(msg_id)
    {
        ...
        case QPPS_DATA_SEND_REQ:
        {
            ...
            if (length > 1)
            {
                ...
                pdu[1] =  QPPS_DATA_SEND_REQ;
				pdu[2] =  param_len+2;
				pdu[3] = (uint8_t)(ATT_SVC_QPPS_DATA);
				pdu[4] = (uint8_t)(ATT_SVC_QPPS_DATA >> 8);
...
//            phOsalNfc_Log("QPPS_DATA_SEND_REQ\r\n");

            //phOsalNfc_Log("Writing bytes : %d \n", length);
            
#if (GDI_GUP_ENABLE == TRUE) // Kanjie
            gup_send_card_evt(pdu, length);
#else
            eaci_uart_write(sizeof(uint8_t) * length, pdu);
#endif
...
//			phOsalNfc_LogCrit("QPPS_DAVA_VAL_IND\r\n");
#ifndef NFC_UNIT_TEST
		...
		case QPPS_CFG_INDNTF_IND:
			phOsalNfc_LogBle("QPPS_CFG_INDNTF_IND\r\n");
			break;

		case QPPS_DATA_SEND_CFM:

			//phOsalNfc_LogCrit("QPPS_DATA_SEND_CFM\r\n");
            phOsalNfc_LogBle("QPPS_DATA_SEND_CFM\r\n");
#ifndef NFC_UNIT_TEST
...
void app_eaci_data_ind(uint8_t msg_id, uint8_t param_len, uint8_t const *param, void* context)
{
  uint16_t uuid;
    ...
    if (param_len < 2)
    {
        ...
        phOsalNfc_LogBle("ERROR in app_eaci_data_ind. BLE Data will not be processed \n");
        return;
    }
...
void app_eaci_data_error_rsp(uint8_t msg_id, uint8_t param_len, uint8_t const *param)
{
    switch (msg_id)
    {
        case EACI_MSG_TYPE_DATA_ERROR:
            if (param[0] == 1)
            {
                phOsalNfc_Log("EACI Command Error.\r\n");
            }
            break;
