...
#define assert(x)     if( ( x ) == 0 ) \
                      {  phOsalNfc_Log("ASSERT:FreeRTOS: Assert Called from file %s Line %d!\r\n", __FILE__, __LINE__ ); \
                         while(1); \
                      }
/****************************************************************************************
 *  ENUM DEFINITIONS
...
/* Neo: move temporary polling receive buffer to stack */
...
static int read_rx_payload_confirm(int dev_index, time_t timeout)
{
	/* Neo: Use polling read for ISP or you can use Chip_UART_Read() with certain timeout */
	uint8_t rxbuf[1];
	Chip_UART_ReadBlocking(BLE_UART_PORT, rxbuf, 1);
...
#if 0
    rx_done = 0;
    ROM_UART_Receive(BLE_UART_PORT, rxbuf, 1);
...
static int read_rx_payload_execute(int dev_index, time_t timeout)
{
#if 0
  int     res = false;
...
static int read_rx_payload_data(int dev_index, pool_t *rx_payload, time_t timeout)
{
    ...
    if (rx_payload->pdata[0] == RSP_DATA_HEAD)
    {
        ...
        if (rx_payload->len >= read_data_behind_len + read_data_front_len)
        {
			...
			//if (CheckRxTimeOut(timeout))
        	{
                ...
                calc_crc16_value = calc_crc16(rx_payload->pdata+ RX_PAYLOAD_DATA_HEAD_LEN,
                                              total_len - RX_PAYLOAD_DATA_HEAD_LEN - CRC16_LEN);
...
//    ROM_UART_Send(BLE_UART_PORT, payload->buffer, payload->len);
    Chip_UART_SendBlocking(BLE_UART_PORT, payload->buffer, payload->len);
    tx_done = 1;
...
static int write_command_wait_rsp_data(int dev_index, pool_t *tx_payload, pool_t **rx_payload, time_t timeout)
{
...
static int write_command_wait_rsp_exe_result(int dev_index, pool_t *tx_payload, time_t timeout)
{
	/* Neo: restore to default design, I don't why Magicoe changes this */
    if (write_tx_payload(dev_index, tx_payload))
    {
        if (read_rx_payload_execute(dev_index, timeout))
            return true;
    }
    return false;
    ...
    if (Chip_UART_SendBlocking(BLE_UART_PORT, tx_payload->buffer, tx_payload->len) != tx_payload->len )
    {
        return false;
    } ;
    tx_done = 1;
...
static int verify_nvds(int dev_index)
{
    ...
    {
        temp_buf = (uint8_t*) phOsalNfc_GetMemory(NVDS_MAX_STORAGE_SIZE);
        assert(temp_buf != NULL);
        res = read_nvds_data(dev_index, temp_buf);
        if (res)
        {
            res = 0 == memcmp(temp_buf,
                              qlib_env[dev_index].nvds.data,
                              qlib_env[dev_index].nvds.length);
        }
        phOsalNfc_FreeMemory(temp_buf);
    }
    else
    {
...
static bool connect(int dev_index, int speed, time_t timeout)
{
	uint8_t  i;
    ...
    {
		Chip_UART_SendBlocking(BLE_UART_PORT, &connect_cmd, sizeof(connect_cmd));
        ...
        if (i == 19)                                            // 8 tries and still no connection
        {
            ...
            phOsalNfc_LogBle("Build connection error\n");
            return false;
        }
    }
    ...
    {
        ERRNO = QE_DEVCIE_IO_WRITE_FAIL;
        phOsalNfc_LogBle("Set baud rate error\n");
        return false;

    }

    BLE_UART_CONFIG(speed);                    // config LPC54102 UART1 baud rate

    /** STEP 4: set both QN902x and LPC5410x baud rates **/
    if (!read_flash_id(dev_index, &flash_id))
    {
        ERRNO = QE_DEVCIE_IO_CLOSE_FAIL;
        phOsalNfc_LogBle("Read Flash ID error\n");
        return false;
    }

    ERRNO = QE_OK;
    phOsalNfc_Log("Connect successfully\n");
    phOsalNfc_Log("Flash ID: %d \n", (int) flash_id);
    return true;
}
...
static void nvds_purge(int dev_index, uint32_t length)
{
  uint8_t  status;
  ...
  uint8_t *buf;
    ...
    buf = (uint8_t*) phOsalNfc_GetMemory(NVDS_MAX_STORAGE_SIZE);
    assert(buf != NULL);
  ...
  const int dev_index = ((qlib_t *)dev)->index;
    ...
    if (status != NVDS_OK)
    {
             ...
             > (NVDS_STORAGE_AREA_SIZE(dev_index) + NVDS_MAGIC_NUMBER_ADDRESS(dev_index)))
        {
            ...
            nvds_purge(dev_index, total_length);
  ...
  int res = false;
    ...
    if (res)
    {
        ...
        if (res)
        {
            ...
            if (res)
            {
                phOsalNfc_Log("NVDS data read successfully\n");
            ...
            else
            {
                ...
                phOsalNfc_LogBle("Read NVDS data error\n");
            }
        }
        else
            ;
...
int qn_api_download_nvds(qdevice_t *dev)
{
  ...
  size_t    nvds_len = qlib_env[dev_index].nvds.length;
    ...
    if (qlib_env[dev_index].nvds_should_purge)
        nvds_purge(dev_index, NVDS_STORAGE_AREA_SIZE(dev_index));
