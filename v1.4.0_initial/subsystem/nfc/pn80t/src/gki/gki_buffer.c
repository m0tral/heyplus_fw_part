...
extern phNfctask_Control_t *gp_nfctask_ctrl;
/*******************************************************************************
**
...
#endif
{
    ...
    /* Make sure the buffers aren't disturbed till finished with allocation */
    GKI_disable();
...
void GKI_freebuf (void *p_buf)
{
    ...
    //LOG_DEBUG("[GKI_freebuf] buf: 0x%x\n", (uint32_t)p_buf );
...
#else
void GKI_send_msg (UINT8 task_id, UINT16 mbox, void *pmsg)
{
    intptr_t bPostStatus = 0;
    ...
    switch(mbox)
    {
        ...
        case NFC_TASK_EVT_TRANSPORT_READY:
    ...
    msg.pMsgData = pmsg;
    if(NFC_TASK == task_id)
        pMsgQ = gp_nfctask_ctrl->pMsgQHandle;

    bPostStatus = phDal4Nfc_msgsnd(pMsgQ, &msg, 0);
#if GKI_BUFFER_DEBUG
    LOGD("GKI_send_msg status %d", bPostStatus);
#endif
}
#endif
/*******************************************************************************
