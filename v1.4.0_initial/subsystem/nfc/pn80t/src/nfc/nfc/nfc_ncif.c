...
extern BOOLEAN MW_RCVRY_FW_DNLD_ALLOWED;
#endif
extern void nfa_hci_rsp_timeout (tNFA_HCI_EVENT_DATA *p_evt_data);

#if (FIRMWARE_DOWNLOAD_ENABLE == TRUE)
/**
 * \brief Sets Actual Firmware Image
 *
 * \return none
 */
extern void SeApi_SetActualFwImage(void);

#endif

#if ((NXP_EXTNS == TRUE) && (FIRMWARE_DOWNLOAD_ENABLE == TRUE))
static void nfc_ncif_recover_nfcc_fw(tNFC_FWUpdate_Info_t* pFw_update_inf, UINT8* pFw_dwnld_status)
{
    //write actual image image
    SeApi_SetActualFwImage();
    nfc_cb.p_hal->ioctl(HAL_NFC_IOCTL_CHECK_FLASH_REQ, pFw_update_inf);
    nfc_cb.p_hal->ioctl(HAL_NFC_IOCTL_FORCED_FW_DWNLD, pFw_dwnld_status);
}
#endif
...
void nfc_ncif_cmd_timeout (void)
{
    ...
    NFC_TRACE_ERROR0 ("Recovery Start!");
    {
        ...
        */
...
#if ((FIRMWARE_DOWNLOAD_ENABLE == TRUE) && (NXP_NFCC_MW_RCVRY_BLK_FW_DNLD == FALSE))
        ...
        buffer = (UINT8 *) phOsalNfc_GetMemory(bufflen*sizeof(UINT8));
        if(NULL == buffer)
        {
        ...
        if (nfc_cb.nfc_state == NFC_STATE_CORE_INIT)
        {
            ...
            nfc_ncif_recover_nfcc_fw(&fw_update_inf, &fw_dwnld_status);
...
UINT8 nfc_ncif_send_data (tNFC_CONN_CB *p_cb, BT_HDR *p_data)
{
    ...
    while (p_data && ((int)(p_data->len) >= 0) && ((int)(p_cb->num_buff) > 0))
    {
...
BOOLEAN nfc_ncif_process_event (BT_HDR *p_msg)
{
    ...
    case NCI_MT_DATA:
        ...
        ry_data_dump(p, 10, ' ');//ry_board_debug_printf("[%s]---%d\n", __FUNCTION__, __LINE__);
        // add by lixueliang 
        ryos_delay_ms(20);
...
void nfc_ncif_proc_reset_rsp (UINT8 *p, BOOLEAN is_ntf)
{
    ...
    if (is_ntf)
    {
        ...
        NFC_TRACE_ERROR1 ("reset notification!! status:0x%x ", status);
        core_reset_init_num_buff = TRUE;
        nfc_ncif_cmd_timeout();
    }
    else
    {
        NFC_TRACE_DEBUG0 ("reset response");
    ...
    if (status == NCI_STATUS_OK)
    {
        ...
        {
#if(NXP_EXTNS == TRUE)
            NFC_TRACE_DEBUG0 ("reset completed, sending core init");
#endif
            nci_snd_core_init ();
        }
        else
...
void nfc_ncif_proc_init_rsp (BT_HDR *p_msg)
{
    ...
    tNFC_FWUpdate_Info_t fw_update_inf;
#if (FIRMWARE_DOWNLOAD_ENABLE == TRUE)
    UINT8 fw_dwnld_status = NCI_STATUS_FAILED;
    UINT8 *init_rsp = NULL ;
#endif
    ...
    if(NCI_STATUS_OK == status)
    {
...
#endif
        {
            ...
            if(fw_update_inf.fw_update_reqd == TRUE)
            {
                ...
                {
                    //if firmware download is failed recover NFCC
                    //by writing required firmware forcefully.
                    nfc_ncif_recover_nfcc_fw(&fw_update_inf, &fw_dwnld_status);
                }
                if(fw_dwnld_status == NCI_STATUS_OK)
                {
...
**
*******************************************************************************/
tNFC_FW_VERSION nfc_ncif_getFWVersion(void)
{
