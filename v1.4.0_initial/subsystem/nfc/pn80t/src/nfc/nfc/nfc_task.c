...
#ifndef ANDROID
phNfctask_Control_t *gp_nfctask_ctrl;
#endif
...
#else
    p_tle->id = GKI_start_timer(NFC_TIMER_ID, GKI_SECS_TO_TICKS (timeout), type);
    p_tle->in_use = TRUE;
#endif
}
...
#else
void nfc_process_timer_evt(UINT16 event)
{
    switch (event)
    {
    case NFC_TTYPE_NCI_WAIT_RSP:
    ...
    case NFC_TTYPE_NCI_WAIT_DATA_NTF:
    {
        if( get_i2c_fragmentation_enabled () == I2C_FRAGMENATATION_ENABLED)
        {
            nfc_cb.i2c_data_t.nci_cmd_channel_busy = 0;
            nfc_cb.i2c_data_t.data_stored = 0;
            LOG_DEBUG("[nfc_process_timer_evt]: channel busy clear.\n");
        }
        nfc_ncif_credit_ntf_timeout();
        break;
    }
    ...
    default:
        NFC_TRACE_DEBUG1 ("nfc_process_timer_evt: event (0x%04x)", event);
        NFC_TRACE_DEBUG1 ("nfc_process_timer_evt: unhandled timer event (0x%04x)", event);
    }
}
#endif
/*******************************************************************************
...
#else
    GKI_stop_timer (NFC_TIMER_ID, p_tle->id);
    p_tle->in_use = FALSE;
#endif
}
...
#else
    GKI_start_timer(NFC_QUICK_TIMER_ID, ((GKI_SECS_TO_TICKS (timeout) / QUICK_TIMER_TICKS_PER_SEC)), type);
    p_tle->in_use = TRUE;
#endif
}
...
#else
    GKI_stop_timer (NFC_QUICK_TIMER_ID, p_tle->id);
    p_tle->in_use = FALSE;
#endif
}
...
void nfc_process_quick_timer_evt (UINT16 event)
{
    switch (event)
    {
    default:
        NFC_TRACE_DEBUG1 ("nfc_process_quick_timer_evt: unhandled timer event (0x%04x)", event);
        break;
    }
}
#endif
...
void nfc_task_shutdown_nfcc (void)
{
    ...
    else
    {
        ...
        /* Stop the timers */
        GKI_stop_timer (NFC_TIMER_ID, 0);
        GKI_stop_timer (NFC_QUICK_TIMER_ID, 0);
#if (defined (NFA_INCLUDED) && NFA_INCLUDED == TRUE)
        GKI_stop_timer (NFA_TIMER_ID, 0);
#endif
    }
}
...
#endif
        /* Add delay to allow Task Switch */
        phOsalNfc_Delay(1);
    }
...
{
    BT_HDR  *p_msg;
    BOOLEAN free_buf, stoptask = FALSE;
    phLibNfc_Message_t msg;

    gp_nfctask_ctrl = (phNfctask_Control_t *) param;
    ...
    while (!stoptask)
    {
        if (phDal4Nfc_msgrcv(gp_nfctask_ctrl->pMsgQHandle, &msg, 0, 0) == -1)
        {
            NFC_TRACE_DEBUG0 ("NFC task received bad message");
            continue;
        }
        if (GKI_SHUTDOWN_EVT_MASK & msg.eMsgType)
        {
           LOG_DEBUG("NFC task stopped.\r\n");
           stoptask = TRUE;
           break;
        }
        /* Handle NFC_TASK_EVT_TRANSPORT_READY from NFC HAL */
        if (NFC_TASK_EVT_TRANSPORT_READY & msg.eMsgType)
        {
            NFC_TRACE_DEBUG0 ("NFC_TASK got NFC_TASK_EVT_TRANSPORT_READY.");
        ...
        if (NFC_MBOX_EVT_MASK & msg.eMsgType)
        {
            /* Process all incoming NCI messages */
            p_msg = (BT_HDR *) msg.pMsgData;
            free_buf = TRUE;
        ...
        /* Process gki timer tick */
        if (NFC_TIMER_EVT_MASK & msg.eMsgType)
        {
            UINT16 event = *((UINT16*)msg.pMsgData);
            nfc_process_timer_evt(event);
        }

        /* Process quick timer tick */
        if (NFC_QUICK_TIMER_EVT_MASK & msg.eMsgType)
        {
            UINT16 event = *((UINT16*)msg.pMsgData);
            nfc_process_quick_timer_evt(event);
        }

#if (defined (NFA_INCLUDED) && NFA_INCLUDED == TRUE)
        if (NFA_MBOX_EVT_MASK & msg.eMsgType)
        {
            nfa_sys_event ((BT_HDR *) msg.pMsgData);
        }

        if (NFA_TIMER_EVT_MASK & msg.eMsgType)
        {
            UINT16 event = *((UINT16*)msg.pMsgData);
            nfa_sys_timer_update(event);
        }
#endif
    }
    NFC_TRACE_DEBUG0 ("nfc_task terminated");
    // notify NfcA_Finalize
    GKI_binary_sem_post(gp_nfctask_ctrl->nfc_task_sem);
    //GKI_exit_task (GKI_get_taskid ());// this function is not required as already its been called from GKI_Shutdown.
