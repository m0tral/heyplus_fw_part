...
void nfc_enabled (tNFC_STATUS nfc_status, BT_HDR *p_init_rsp_msg)
{
    ...
    else if(nfc_status == NCI_STATUS_UNEXPECTED_FW)
    {
        nfc_set_state(NFC_STATE_IDLE);
    }
            ...
            */
            if (nfc_cb.nfc_state == NFC_STATE_W4_POST_INIT_CPLT)
            {
                if (p_msg->status == HAL_NFC_STATUS_OK)
                {
                     ...
                     * */
                    mGetCfg_info = (phNxpNci_getCfg_info_t*)GKI_os_malloc(sizeof(phNxpNci_getCfg_info_t));
                    if(mGetCfg_info != NULL)
                    {
                        ...
                        GKI_os_free(mGetCfg_info);
                        mGetCfg_info = NULL;
                    }
#endif
                }
                else if(p_msg->status == HAL_NFC_STATUS_INVALID_FW)
                {
                    nfc_enabled (NCI_STATUS_UNEXPECTED_FW, NULL);
                }
                          ...
                          BT_HDR     *p_data)
{
    ...
    {
        //p_cb->num_buff = NFC_CONN_NO_FC;//Added Temporarily as init was failing due to credit notification timeout.
                                          //As init works in release mode Data Flow Control Needs to be enabled
