...
#if (NXP_EXTNS == TRUE)
#if 0 /* Warning fix - Unused functions */
...
#endif /* Warning fix - Unused functions */
...
void nfa_hci_handle_admin_gate_rsp (UINT8 *p_data, UINT8 data_len)
{
    ...
    {
        nfa_sys_stop_timer (&nfa_hci_cb.timer); /* Wearable fix: Added this to stop the OSAL timer as response received
                                                   This can cause double stops for timer from places such as
                                                   nfa_hci_startup_complete */
        ...
        if (nfa_hci_cb.inst != NFA_HCI_ANY_OK)
        {
            ...
            {
                if(nfa_hci_cb.cmd_sent == NFA_HCI_ANY_SET_PARAMETER)
                {
...
tNFA_STATUS nfa_hci_api_config_nfcee (UINT8 hostId)
{
    ...
    //UINT8               count=0; //Unused
    UINT8               pipeId=0;
    //BOOLEAN             bCreatepipe = FALSE; //Unused
...
{
    //UINT8 discovered_num_nfcee = NFA_HCI_MAX_HOST_IN_NETWORK; //Unused
    UINT8 xx;
    tNFA_STATUS         status = NFA_STATUS_OK;
    //UINT8 host_index = 0x00; //Unused
    for ( xx = 0; xx < nfa_hci_cb.num_nfcee; xx++)
    {
        if ((nfa_hci_cb.hci_ee_info[xx].num_interface != 0) &&
            (nfa_hci_cb.hci_ee_info[xx].ee_interface[0] != NCI_NFCEE_INTERFACE_HCI_ACCESS) &&
...
static void nfa_hci_poll_session_id_cb (UINT8 event, UINT16 param_len, UINT8 *p_param)
{
    ...
    //if (1)
    {
        ...
        LOG_ERROR ("[nfa_hci_poll_session_id_cb] Invalid session id, Need to reset\r\n");
    ...
    else
    {
        ...
        LOG_DEBUG ("nfa_hci_poll_session_id_cb valid session id\r\n"  );
...
static void nfa_hci_handle_Nfcee_admpipe_rsp (UINT8 *p_data, UINT8 data_len)
{
    ...
    else if ((nfa_hci_cb.cmd_sent == NFA_HCI_ANY_GET_PARAMETER)&& (nfa_hci_cb.inst == NFA_HCI_ANY_OK))
    {
            ...
            {
                /*nfa_sys_stop_timer (&nfa_hci_cb.timer);*//* Wearable fix: Added this in the caller of this function
                                                              as its applicable for all commands */
                ...
                NFA_TRACE_DEBUG1("nfa_hci_handle_admin_gate_rsp  no of hosts : state %d",evt_data.admin_rsp_rcvd.status);
            }
    }
...
static void nfa_hci_handle_Nfcee_dynpipe_rsp (UINT8 pipeId,UINT8 *p_data, UINT8 data_len)
{
    ...
    //tNFA_HCI_DYN_GATE   *p_gate; //Unused
    tNFA_STATUS         status = NFA_STATUS_FAILED;
    ...
    case NFA_HCI_ANY_OPEN_PIPE:
            ...
            if(p_pipe&&(p_pipe-> dest_gate == NFA_HCI_IDENTITY_MANAGEMENT_GATE )&&(p_pipe-> local_gate == NFA_HCI_IDENTITY_MANAGEMENT_GATE))
            {
...
static BOOLEAN nfa_hci_api_GetpipeId(UINT8 nfceeId,UINT8 gateId,UINT8 *pipeId)
{
    ...
    for (count = 0;count < NFA_HCI_MAX_PIPE_CB;count++)
    {
        if(((nfa_hci_cb.cfg.dyn_pipes[count].dest_host)== nfceeId) && ((nfa_hci_cb.cfg.dyn_pipes[count].dest_gate)== gateId)
                                 &&((nfa_hci_cb.cfg.dyn_pipes[count].local_gate)== gateId))
        {
            NFA_TRACE_DEBUG1 ("nfa_hci_api_GetpipeId -pipeId -- %d !!!",nfa_hci_cb.cfg.dyn_pipes[count].pipe_id);
