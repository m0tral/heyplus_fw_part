...
void nfa_hci_ee_info_cback (tNFA_EE_DISC_STS status)
{
    ...
    case NFA_EE_DISC_STS_REQ:
        ...
        {
            nfa_sys_stop_timer (&nfa_hci_cb.timer);
            nfa_sys_start_timer (&nfa_hci_cb.timer, NFA_HCI_RSP_TIMEOUT_EVT, 2000);/* Wearable fix: 150ms was not sufficient with full logs enabled */
        }
#endif
        if (nfa_hci_cb.ee_disable_disc)
        {
...
//NFC_INIT MACH
    ...
    /* Wearable fix: nfa_hci_cb.num_ee_dis_req_ntf != (nfa_hci_cb.num_nfcee - 1) is changed to
     * nfa_hci_cb.num_ee_dis_req_ntf < (nfa_hci_cb.num_nfcee - 1)*NFA_HCI_NUM_DISCREQNTFS_PER_NFCEE
     * As per the User Manual, there should be single disc_req_ntf per NFCEE. However, FW is giving
     * one notification for each technology. Therefore, this count is not comparable with NFCEE count
     * For now, as we can't know how many notifications will arrive per NFCEE, adding the check with assumption
     * that same number of technologies supported by each NFCEE */
    else if (  (nfa_hci_cb.num_nfcee > 1)
             &&(nfa_hci_cb.num_ee_dis_req_ntf < (nfa_hci_cb.num_nfcee - 1) * NFA_HCI_NUM_DISCREQNTFS_PER_NFCEE)  )
    {
        if (nfa_hci_cb.hci_state == NFA_HCI_STATE_RESTORE)
            nfa_hci_cb.ee_disable_disc  = TRUE;
        /* Received HOT PLUG EVT, we will also wait for EE DISC REQ Ntf(s) */
        /* Wearable fix : When JCOP Download is interrupted, during next init, ESE comes as enabled but however
         * DISC_REQ_NTFs are not given. So, starting a timer would obviously timeout. Start timer if atleast one NTF
         * has come so that we wait for rest of the NTFs */
        if(nfa_hci_cb.num_ee_dis_req_ntf)
            nfa_sys_start_timer (&nfa_hci_cb.timer, NFA_HCI_RSP_TIMEOUT_EVT, p_nfa_hci_cfg->hci_netwk_enable_timeout);
    }
    else
    {
...
void nfa_hci_startup_complete (tNFA_STATUS status)
{
    ...
    else if(nfa_hci_cb.hci_state == NFA_HCI_STATE_NFCEE_ENABLE)
    {
        ...
        {
            /* Wearable fix: For query of APDU gate registry to be successful, its required that POWER and LINK are
             * configured to be always ON before starting with the eSE initialization sequence.
             * However, in the case of MODE_SET_ON power mode is not touched. So, this query is expected to timeout.
             * Android doesn't treat timeout as failure. So, this fix becomes wearable specific
             */
            if((nfa_hci_cb.cmd_sent == NFA_HCI_ANY_GET_PARAMETER)&& (nfa_hci_cb.param_in_use == NFA_HCI_MAX_C_APDU_SIZE_INDEX))
...
static void nfa_hci_sys_enable (void)
{
...
#else
    {
        ...
        memcpy (&session_id[(NFA_HCI_SESSION_ID_LEN / 2)], reset_session, (NFA_HCI_SESSION_ID_LEN / 2));
        ...
        nfa_hci_cb.nv_read_cmplt = TRUE; //needed by nfa_hci_startup
        nfa_hci_startup ();
    }
#endif
}
...
void nfa_hci_conn_cback (UINT8 conn_id, tNFC_CONN_EVT event, tNFC_CONN *p_data)
{
    ...
    /* Wearable fix: Added this to stop the OSAL timer as response received - Start */
    if (event == NFC_CONN_CREATE_CEVT || event == NFC_CONN_CLOSE_CEVT)
    {
        nfa_sys_stop_timer (&nfa_hci_cb.timer);
    }
    /* Wearable fix: Added this to stop the OSAL timer as response received - End */
    ...
    if (event == NFC_CONN_CREATE_CEVT)
    {
        ...
        /* Find entry for HCI network */
        nfa_hci_cb.num_nfcee = NFA_EE_MAX_EE_SUPPORTED;
        NFA_AllEeGetInfo (&nfa_hci_cb.num_nfcee, nfa_hci_cb.hci_ee_info);
        for ( xx = 0; xx < nfa_hci_cb.num_nfcee; xx++)
        {
            if (nfa_hci_cb.hci_ee_info[xx].ee_handle == 0x401)
            {
                break;
            }
        }

        if(nfa_hci_cb.hci_ee_info[xx].ee_status  == NFA_EE_STATUS_INACTIVE) /* State of HCI n/w is disabled */
        {
            if (nfa_hci_cb.cfg.admin_gate.pipe01_state == NFA_HCI_PIPE_CLOSED)
            {
                /* First step in initialization/restore is to open the admin pipe */
...
static void read_config_timeout_param_values()
{
    if(GetNxpNumValue(CFG_NXP_DEFAULT_NFCEE_TIMEOUT, (void*)&nfa_hci_cb.max_hci_session_id_read_count, sizeof(nfa_hci_cb.max_hci_session_id_read_count)) == FALSE)
    {
    ...
    if(GetNxpNumValue(CFG_NXP_DEFAULT_NFCEE_DISC_TIMEOUT, (void *)&nfa_hci_cb.max_nfcee_disc_timeout, sizeof(nfa_hci_cb.max_nfcee_disc_timeout)) == FALSE)
    {
