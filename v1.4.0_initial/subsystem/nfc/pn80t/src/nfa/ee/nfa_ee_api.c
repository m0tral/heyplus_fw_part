...
/*******************************************************************************
**
** Function         NFA_EeSetPwrLink
**
** Description      This function is called to send power link command
**                  to NFCC. ee_handle is the handle to NFCEE and
**                  pwr_cfgis the power configuration to be enabled.
**
** Returns          NFA_STATUS_OK if successfully initiated
**                  NFA_STATUS_FAILED otherwise
**                  NFA_STATUS_INVALID_PARAM If bad parameter
**
*******************************************************************************/
tNFA_STATUS NFA_EeSetPwrLink(tNFA_HANDLE    ee_handle,
                             tNFA_EE_PWR_CFG     pwr_cfg)
{
    tNFA_EE_API_SET_PWR_LINK *p_msg;
    tNFA_STATUS status = NFA_STATUS_FAILED;
    tNFA_EE_ECB *p_cb, *p_found = NULL;
    UINT32  xx;
    UINT8   nfcee_id = (ee_handle & 0xFF);
    ...
    NFA_TRACE_API2 ("NFA_EeSetPwrLink(): handle:<0x%x>, pwr_cfg:0x%02X", ee_handle, pwr_cfg);

    if (p_found == NULL)
    {
        NFA_TRACE_ERROR1 ("NFA_EeSetPwrLink() invalid NFCEE:0x%04x", ee_handle);
        status = NFA_STATUS_INVALID_PARAM;
    }
    else if ((p_msg = (tNFA_EE_API_SET_PWR_LINK *) GKI_getbuf (sizeof(tNFA_EE_API_SET_PWR_LINK))) != NULL)
    {
        p_msg->hdr.event        = NFA_EE_API_SET_PWR_LINK_EVT;
        p_msg->nfcee_id         = nfcee_id;
        p_msg->pwr_cfg          = pwr_cfg;
