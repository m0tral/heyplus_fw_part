...
#if NFA_ENABLE_AID_ROUTING == TRUE
...
#if NFA_ENABLE_AID_ROUTING == TRUE
#if((NXP_EXTNS == TRUE) && (NFC_NXP_CHIP_TYPE == PN548C2) && (NFC_NXP_AID_MAX_SIZE_DYN == TRUE))
    for(xx = 0; xx < NFA_EE_NUM_ECBS; xx++)
    {
...
/*******************************************************************************
**
** Function         nfa_ee_api_set_nci_pwr_link_ctrl
**
** Description      process set power link configuration from user
**                  based on the power config (NFCC_DECIDES,POWER_ALWAYS_ON,LINK_ALWAYS_ON)
**                  power link command is sent to NFCC
**
** Returns          void
**
*******************************************************************************/
void nfa_ee_api_set_nci_pwr_link_ctrl(tNFA_EE_MSG *p_data)
{
    nci_snd_pwr_nd_lnk_ctrl_cmd(p_data->set_pwr_link.nfcee_id, p_data->set_pwr_link.pwr_cfg);
}
...
void nfa_ee_report_disc_done(BOOLEAN notify_enable_done)
{
    ...
    if (nfa_ee_cb.num_ee_expecting == 0)
    {
        ...
        {
        	// p_cback of NULL will cause reporting to all installed callbacks.
            evt_data.status                         = NFA_STATUS_OK;
            evt_data.ee_discover.num_ee             = NFA_EE_MAX_EE_SUPPORTED;
...
tNFA_STATUS nfa_ee_route_add_one_ecb(tNFA_EE_ECB *p_cb, int *p_max_len, BOOLEAN more, UINT8 *ps, int *p_cur_offset)
{
...
#else
    NFA_TRACE_DEBUG4 ("nfa_ee_route_add_one_ecb max_len:%d, cur_offset:%d, more:%d, num_tlv:%d",
        *p_max_len, *p_cur_offset, more, num_tlv);
#endif
...
#endif
    {
        ...
        for (xx = 0; xx < p_cb->aid_entries; xx++)
        {
            p_start     = pp; /* remember the beginning of this AID routing entry, just in case we need to put it in next command */
            /* add one AID entry */
#if(NXP_EXTNS == TRUE)
            if ((p_cb->aid_rt_info[xx] & NFA_EE_AE_ROUTE) &&
...
#endif/*NFA_ENABLE_AID_ROUTING*/
#if NFA_ENABLE_PROTOCOL_ROUTING == TRUE
...
#endif/*NFA_ENABLE_PROTOCOL_ROUTING*/
...
void find_and_resolve_tech_conflict()
{
    ...
    UINT8   techA_ee = NFA_EE_INVALID, techF_ee = NFA_EE_INVALID;
    ...
    if((GetNumValue(CFG_DEFAULT_OFFHOST_ROUTE, &preferred_se, sizeof(preferred_se))))
    {
