...
#define NFA_EE_ROUT_BUF_SIZE            80 /* Maximum configuration size required for routing is only 80 bytes for wearable */
#else
#define NFA_EE_ROUT_BUF_SIZE            720
#endif /* WEARABLE CORE SDK */
#else
...
/* data type for NFA_EE_API_SET_PWR_LINK_EVT */
typedef struct
{
    BT_HDR             hdr;
    UINT8              nfcee_id;
    UINT8              pwr_cfg;
} tNFA_EE_API_SET_PWR_LINK;
...
typedef union
{
    ...
    tNFA_EE_API_SET_PWR_LINK    set_pwr_link;
