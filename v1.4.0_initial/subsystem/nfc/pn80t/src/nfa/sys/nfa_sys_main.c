...
tNFA_SYS_CB nfa_sys_cb = {0, .trace_level = 5};   /* nfa_sys control block. statically initialize 'flags' field to 0 */
#endif
...
#else
void nfa_sys_timer_update(UINT16 event)
{
    BT_HDR *p_msg;
...
{
    nfa_sys_ptim_start_timer (&nfa_sys_cb.ptim_cb, p_tle, type, timeout*10);
}
