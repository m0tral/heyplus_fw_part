...
#else
   // NFA_TRACE_DEBUG1 ("nfa_sys_ptim_start_timer 0x%04X", type);
    p_tle->id = GKI_start_timer(p_cb->timer_id, GKI_MS_TO_TICKS (timeout), type);
    p_tle->in_use = TRUE;
#endif
}
...
#else
    NFA_TRACE_DEBUG0 ("ptim timer stop");
    GKI_stop_timer (p_cb->timer_id, p_tle->id);
    p_tle->in_use = FALSE;
#endif
}
