...
//    }
    phOsalNfc_SetMemory(&gLog_level, level, sizeof(nci_log_level_t));
    return level;
}
...
static void phNxpLog_SetHALLogLevel (uint8_t level)
{
    ...
    if (GetNxpNumValue (CFG_NXPLOG_NCIHAL_LOGLEVEL, &num, sizeof(num)))
    {
...
//    int len;
//    char valueStr [PROPERTY_VALUE_MAX] = {0};
    if (GetNxpNumValue (CFG_NXPLOG_EXTNS_LOGLEVEL, &num, sizeof(num)))
    {
...
//    int len;
//    char valueStr [PROPERTY_VALUE_MAX] = {0};
    if (GetNxpNumValue (CFG_NXPLOG_TML_LOGLEVEL, &num, sizeof(num)))
    {
...
//    int len;
//    char valueStr [PROPERTY_VALUE_MAX] = {0};
    if (GetNxpNumValue (CFG_NXPLOG_FWDNLD_LOGLEVEL, &num, sizeof(num)))
    {
...
//    int len;
//    char valueStr [PROPERTY_VALUE_MAX] = {0};
    if (GetNxpNumValue (CFG_NXPLOG_NCIX_LOGLEVEL, &num, sizeof(num)))
    {
        gLog_level.ncix_log_level = (level > (unsigned char) num) ? level : (unsigned char) num;
    }
    if (GetNxpNumValue (CFG_NXPLOG_NCIR_LOGLEVEL, &num, sizeof(num)))
    {
...
#ifdef LOGS_TO_CONSOLE /* Reduces the logs when routing logs to console */
    gLog_level.global_log_level = NXPLOG_LOG_ERROR_LOGLEVEL; /* Set to Error level */
    gLog_level.dnld_log_level = \
    gLog_level.extns_log_level = \
    gLog_level.hal_log_level = \
    gLog_level.tml_log_level = gLog_level.global_log_level;
#endif
