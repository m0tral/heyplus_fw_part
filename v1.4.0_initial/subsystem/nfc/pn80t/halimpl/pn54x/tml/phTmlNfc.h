...
typedef struct phTmlNfc_Context
{
    ...
    uintptr_t dwCallbackTaskHandle;    /* Callback task to which message to be posted */
    uint8_t bEnableCrc; /*Flag to validate/not CRC for input buffer */
    void *rxSemaphore;
    void *txSemaphore; /* Lock/Aquire txRx Semaphore */
    void *postMsgSemaphore; /* Semaphore to post message atomically by Reader & writer thread */
    volatile uint8_t bWait_busy_flag;/*Condition flag to wait reader thread*/
    volatile uint8_t  bWriterCbflag; /* flag to indicate write callback message is pushed to queue*/
    void *wait_busy_condition;/*condition for reader thread to wait till write operation is complete*/
    void *wait_busy_lock;/*For reader thread to wait till write operation is complete*/
} phTmlNfc_Context_t;
...
typedef struct phTmlNfc_Config
{
    ...
    /* Callback Thread ID
     *
     * This is the Task Handle on which the Reader & Writer thread posts message. */
    uintptr_t dwGetMsgTaskHandle;
...
typedef struct {
     ...
     */
    uint32_t dwMsgPostedTask;   //use TaskHandle instead of thread ID
    /** Actual Message
     *
...
phTmlNfc_i2cfragmentation_t phTmlNfc_get_fragmentation_enabled(void);
#endif /*  PHTMLNFC_H  */
