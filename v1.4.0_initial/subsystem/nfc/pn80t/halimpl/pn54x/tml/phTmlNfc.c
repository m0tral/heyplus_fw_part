...
#define TML_READ_WRITE_SYNC_TIMEOUT		(2)
...
static NFCSTATUS phTmlNfc_StartTask(void);
...
static void phTmlNfc_TmlTask(void *pParam);
static void phTmlNfc_TmlWriterTask(void *pParam);
...
NFCSTATUS phTmlNfc_Init(pphTmlNfc_Config_t pConfig)
{
    ...
    /* Validate Input parameters */
            ...
            (PH_TMLNFC_RESET_VALUE == pConfig->dwGetMsgTaskHandle))
    {
    ...
    else
    {
        ...
        gpphTmlNfc_Context = phOsalNfc_GetMemory(sizeof(phTmlNfc_Context_t));
        ...
        else
        {
            ...
            wInitStatus = phTmlNfc_i2c_open_and_configure(&(gpphTmlNfc_Context->pDevHandle));
            ...
            else
            {
                ...
                wInitStatus = phOsalNfc_CreateSemaphore(&gpphTmlNfc_Context->rxSemaphore, 0);
                if(wInitStatus == NFCSTATUS_SUCCESS)
                {
                    wInitStatus = phOsalNfc_CreateSemaphore(&gpphTmlNfc_Context->txSemaphore, 0);
                    {
                        if(wInitStatus == NFCSTATUS_SUCCESS)
                        {
                            wInitStatus = phOsalNfc_CreateSemaphore(&gpphTmlNfc_Context->postMsgSemaphore, 0);
                            if(wInitStatus == NFCSTATUS_SUCCESS)
                            {
                                wInitStatus = phOsalNfc_CreateSemaphore(&gpphTmlNfc_Context->wait_busy_condition, 0);
                                if(wInitStatus == NFCSTATUS_SUCCESS)
                                {
                                    wInitStatus = phOsalNfc_CreateMutex(&gpphTmlNfc_Context->wait_busy_lock);
                                }
                            }
                        }
                    }
                ...
                if(wInitStatus == NFCSTATUS_SUCCESS)
                {
                    phOsalNfc_ProduceSemaphore(gpphTmlNfc_Context->postMsgSemaphore);
                     /* Start TML thread (to handle write and read operations) */
                    if (NFCSTATUS_SUCCESS != phTmlNfc_StartTask())
                    {
                        wInitStatus = PHNFCSTVAL(CID_NFC_TML, NFCSTATUS_FAILED);
                    }
                    else
                    {
                        ...
                        gpphTmlNfc_Context->dwTimerId = phOsalNfc_Timer_Create(FALSE);
                        if (PH_OSALNFC_TIMER_ID_INVALID != gpphTmlNfc_Context->dwTimerId)
                        {
                            /* Store the Thread Identifier to which Message is to be posted */
                            gpphTmlNfc_Context->dwCallbackTaskHandle = pConfig->dwGetMsgTaskHandle;
...
static NFCSTATUS phTmlNfc_StartTask(void)
{
    ...
    int readstacksize = TMLREAD_STACK_SIZE;
    int writestacksize = TMLWRITE_STACK_SIZE;

     threadparams.stackdepth = readstacksize;
     strcpy((char *)threadparams.taskname, "readerTask");
     threadparams.pContext = NULL;
     threadparams.priority = 3;

      //Create Reader and Writer threads
     if(phOsalNfc_Thread_Create(&gpphTmlNfc_Context->readerTask, phTmlNfc_TmlTask, &threadparams) != 0)
     {
         wStartStatus = NFCSTATUS_FAILED;
     }
     else
     {
         threadparams.stackdepth = writestacksize;
         strcpy((char *)threadparams.taskname, "writerTask");
         threadparams.pContext = NULL;
         threadparams.priority = 3;
         if(phOsalNfc_Thread_Create(&gpphTmlNfc_Context->writerTask, phTmlNfc_TmlWriterTask, &threadparams) != 0)
         {
            wStartStatus = NFCSTATUS_FAILED;
         }
     }
...
static void phTmlNfc_ReTxTimerCb(uint32_t dwTimerId, void *pContext)
{
            ...
            (NULL == pContext))
    {
        ...
        phOsalNfc_ProduceSemaphore(gpphTmlNfc_Context->txSemaphore);
    }
...
static void phTmlNfc_TmlTask(void *pParam)
{
    ...
    UNUSED(pParam);
    NXPLOG_TML_D("PN54X - Tml Reader Task Started................\n");
    ...
    while (gpphTmlNfc_Context->bThreadDone)
    {
        ...
        phOsalNfc_ConsumeSemaphore(gpphTmlNfc_Context->rxSemaphore);
        ...
        if (1 == gpphTmlNfc_Context->tReadInfo.bEnable)
        {
            ...
            if (((uintptr_t)gpphTmlNfc_Context->pDevHandle) > 0)
            {
                ...
                dwNoBytesWrRd = phTmlNfc_i2c_read(temp, 260);
                ...
                if (-1 == dwNoBytesWrRd)
                {
                    ...
                    else
                    {
                        ...
                        phTmlNfc_DeferredCall(gpphTmlNfc_Context->dwCallbackTaskHandle, &tMsg);
                        return;
                    }
                    phOsalNfc_ProduceSemaphore(gpphTmlNfc_Context->rxSemaphore);
                }
                ...
                else
                {
                    ...
                    if (gpphTmlNfc_Context->tReadInfo.pBuffer[0] == 0x61 && 
                        gpphTmlNfc_Context->tReadInfo.pBuffer[1] == 0x05 &&
										    gpphTmlNfc_Context->tReadInfo.wLength >= 17 ) {
                        LOG_DEBUG("NFC ID Got.\r\n");
                        ry_data_dump(&gpphTmlNfc_Context->tReadInfo.pBuffer[13], 4, ' ');
                    }
                    ...
                    if((gpphTmlNfc_Context->bWriterCbflag == FALSE) &&
...
/*******************************************************************************
**
** Function         phTmlNfc_TmlWriterTask
**
...
static void phTmlNfc_TmlWriterTask(void *pParam)
{
    ...
    UNUSED(pParam);
    NXPLOG_TML_D("PN54X - Tml Writer Task Started................\n");

    /* Add an infinite loop to disallow task function from exiting. This can happen when
     * 1. task is provided with txSemaphore
     * 2. gpphTmlNfc_Context->bThreadDone is false and
     * 3. phTmlNfc_i2c_write timesout / there is no write request
     * Its responsibility of caller, who sets gpphTmlNfc_Context->bThreadDone to false, to delete this task
     */
    for(;;) {
    /* Writer thread loop shall be running till shutdown is invoked */
    while (gpphTmlNfc_Context->bThreadDone)
    {
        NXPLOG_TML_D("PN54X - Tml Writer Task Running................\n");
        phOsalNfc_ConsumeSemaphore(gpphTmlNfc_Context->txSemaphore);
        /* If Tml write is requested */
        if (1 == gpphTmlNfc_Context->tWriteInfo.bEnable)
        {
            ...
            if (((uintptr_t)gpphTmlNfc_Context->pDevHandle) > 0)
            {
                ...
                gpphTmlNfc_Context->bWriterCbflag = FALSE;
                dwNoBytesWrRd = phTmlNfc_i2c_write(gpphTmlNfc_Context->tWriteInfo.pBuffer,
                ...
                else
                {
                    ...
                    if (NFCSTATUS_SUCCESS == wStatus)
                    {
                        gpphTmlNfc_Context->bWriterCbflag = TRUE;
...
static void phTmlNfc_CleanUp(void)
{
   // NFCSTATUS wRetval = NFCSTATUS_SUCCESS;//unused

    if (NULL != gpphTmlNfc_Context->pDevHandle)
    {
        (void) phTmlNfc_i2c_reset(0);
        gpphTmlNfc_Context->bThreadDone = 0;
    }
    phOsalNfc_DeleteSemaphore(&gpphTmlNfc_Context->rxSemaphore);
    gpphTmlNfc_Context->rxSemaphore = NULL;
    phOsalNfc_DeleteSemaphore(&gpphTmlNfc_Context->txSemaphore);
    gpphTmlNfc_Context->txSemaphore = NULL;
    phOsalNfc_DeleteSemaphore(&gpphTmlNfc_Context->postMsgSemaphore);
    gpphTmlNfc_Context->postMsgSemaphore = NULL;
    phOsalNfc_DeleteSemaphore(&gpphTmlNfc_Context->wait_busy_condition);
    gpphTmlNfc_Context->wait_busy_condition = NULL;
    phOsalNfc_DeleteMutex(&gpphTmlNfc_Context->wait_busy_lock);
    gpphTmlNfc_Context->wait_busy_lock = NULL;
    phTmlNfc_i2c_close(gpphTmlNfc_Context->pDevHandle);
    gpphTmlNfc_Context->pDevHandle = NULL;
    /* Clear memory allocated for storing Context variables */
    phOsalNfc_FreeMemory((void *) gpphTmlNfc_Context);
...
NFCSTATUS phTmlNfc_Shutdown(void)
{
    ...
    if (NULL != gpphTmlNfc_Context)
    {
        ...
        phOsalNfc_Thread_Delete(gpphTmlNfc_Context->readerTask);
        phOsalNfc_Thread_Delete(gpphTmlNfc_Context->writerTask);
    ...
    if (NULL != gpphTmlNfc_Context)
    {
                ...
                (PH_TMLNFC_RESET_VALUE != wLength) && (NULL != pTmlWriteComplete))
        {
            if (!gpphTmlNfc_Context->tWriteInfo.bThreadBusy)
            {
                ...
                //If retry is going on. Stop the retry thread/timer (to be checked)
                if (phTmlNfc_e_EnableRetrans == gpphTmlNfc_Context->eConfig)
                {
                    /* Set retry count to default value */
                    // If the timer expired there, and meanwhile we have created (to be checked)
...
void phTmlNfc_DeferredCall(uintptr_t dwThreadId, phLibNfc_Message_t *ptWorkerMsg)
{
    ...
    phOsalNfc_ConsumeSemaphore(gpphTmlNfc_Context->postMsgSemaphore);
    phDal4Nfc_msgsnd((int) (gpphTmlNfc_Context->dwCallbackTaskHandle),
                ptWorkerMsg,
                0
                );
    phOsalNfc_ProduceSemaphore(gpphTmlNfc_Context->postMsgSemaphore);
...
static void phTmlNfc_WaitWriteComplete(void)
{
    phOsalNfc_LockMutex(gpphTmlNfc_Context->wait_busy_lock);
    gpphTmlNfc_Context->bWait_busy_flag = TRUE;
    phOsalNfc_UnlockMutex(gpphTmlNfc_Context->wait_busy_lock);
    NXPLOG_TML_D("phTmlNfc_WaitWriteComplete - enter");
    if( phOsalNfc_ConsumeSemaphore_WithTimeout(gpphTmlNfc_Context->wait_busy_condition, TML_READ_WRITE_SYNC_TIMEOUT) != NFCSTATUS_SUCCESS)
    {
        phOsalNfc_LockMutex(gpphTmlNfc_Context->wait_busy_lock);
        gpphTmlNfc_Context->bWait_busy_flag = FALSE;
        phOsalNfc_UnlockMutex(gpphTmlNfc_Context->wait_busy_lock);
...
static void phTmlNfc_SignalWriteComplete(void)
{
    if (gpphTmlNfc_Context->bWait_busy_flag == TRUE)
    {
        NXPLOG_TML_D("phTmlNfc_SignalWriteComplete - enter");
        phOsalNfc_LockMutex(gpphTmlNfc_Context->wait_busy_lock);
        gpphTmlNfc_Context->bWait_busy_flag = FALSE;
        phOsalNfc_UnlockMutex(gpphTmlNfc_Context->wait_busy_lock);
        phOsalNfc_ProduceSemaphore(gpphTmlNfc_Context->wait_busy_condition);
