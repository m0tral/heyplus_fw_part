...
static BOOLEAN force_fw_download_req = FALSE;
...
/*static void
phNxpNciHal_fw_dnld_send_ncicmd_Cb(void* pContext, NFCSTATUS status, void* pInfo);*/// not defined
        ...
        void* pInfo)
{
    ...
    else if(NFCSTATUS_FW_SIGNATURE_MISMATCH == status)
    {
        NXPLOG_FWDNLD_E("Signature Mismatch!!");
        (gphNxpNciHal_fw_IoctlCtx.bDnldRecovery) = FALSE;
        (gphNxpNciHal_fw_IoctlCtx.bRetryDnld) = FALSE;
    }
...
void  phNxpNciHal_set_forced_dnd(BOOLEAN forcedDnd)
{
    force_fw_download_req = forcedDnd;
}
...
**
*******************************************************************************/
NFCSTATUS phNxpNciHal_fw_download_seq(uint8_t bClkSrcVal, uint8_t bClkFreqVal, BOOLEAN forcedDnd)
{
    NFCSTATUS status = NFCSTATUS_FAILED;
    phDnldNfc_Buff_t pInfo;
    char *pContext = "FW-Download";
    ...
    (gphNxpNciHal_fw_IoctlCtx.bChipVer) = 0;
    (gphNxpNciHal_fw_IoctlCtx.bSkipSeq) = FALSE;
    (gphNxpNciHal_fw_IoctlCtx.bForceDnld) = forcedDnd;
    (gphNxpNciHal_fw_IoctlCtx.bDnldRecovery) = FALSE;
    (gphNxpNciHal_fw_IoctlCtx.bRetryDnld) = FALSE;
    (gphNxpNciHal_fw_IoctlCtx.bSkipReset) = FALSE;
    (gphNxpNciHal_fw_IoctlCtx.bSkipForce) = FALSE;
