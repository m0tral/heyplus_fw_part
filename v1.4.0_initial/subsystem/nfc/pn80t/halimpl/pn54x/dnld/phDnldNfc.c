...
pphDnldNfc_DlContext_t gpphDnldContext = NULL; /* Download contex */
//static pphDnldNfc_RspCb_t      UserCb; /* Upper layer call back function *///unused
//static void*                   UserCtxt; /* Pointer to upper layer context */ //unused
#undef EEPROM_Read_Mem_IMP

extern uint8_t *gpFwBuffer;
extern uint32_t gFwBufferSize;
extern uint8_t *gpFwRecBuffer;
extern uint32_t gFwRecBufferSize;
    ...
    else
    {
        ...
        else
        {
            if((NULL != pData->pBuff) && (0 != pData->wLen))
            {
                ...
                phOsalNfc_SetMemory(&(gpphDnldContext->tRWInfo),0,sizeof(gpphDnldContext->tRWInfo));
...
void phDnldNfc_SetHwDevHandle(void)
{
    ...
    if(NULL == gpphDnldContext)
    {
        ...
        psDnldContext = (pphDnldNfc_DlContext_t)
						phOsalNfc_GetMemory(sizeof(phDnldNfc_DlContext_t));

        if(psDnldContext != NULL)
        {
            (void ) phOsalNfc_SetMemory((void *)psDnldContext,0,
    ...
    {
        (void ) phOsalNfc_SetMemory((void *)gpphDnldContext,0,
                                            sizeof(phDnldNfc_DlContext_t));
    }
    return;
}
...
void phDnldNfc_ReSetHwDevHandle(void)
{
    if (gpphDnldContext != NULL)
    {
		...
		phOsalNfc_FreeMemory(gpphDnldContext);
...
NFCSTATUS phDnldNfc_InitImgInfo(void)
{
    ...
    if(GetNxpStrValue(CFG_NXP_FW_NAME, fwFileName, sizeof(fwFileName)) == TRUE)
    {
    ...
    if(gRecFWDwnld == TRUE)
    {
        ...
        // gpFwRecBuffer and gFwRecBufferSize must be set at the top layer before the download
        // is started.
        pImageInfo = gpFwRecBuffer;
        ImageInfoLen = gFwRecBufferSize;
    }
    else
...
NFCSTATUS phDnldNfc_LoadFW(const char* pathName, uint8_t **pImgInfo, uint16_t* pImgInfoLen)
{
    ...
    if(0 != strcmp(pathName, "/system/vendor/firmware/libpn548ad_fw.so"))
    {
        NXPLOG_FWDNLD_D("Only libpn548ad_fw.so is supported");
    ...
    if(0 != strcmp(pathName, "/system/vendor/firmware/libpn547_fw.so"))
    {
        NXPLOG_FWDNLD_D("Only libpn548ad_fw.so is supported");
        return NFCSTATUS_FAILED;
    }
    ...
    if(NULL == gpFwBuffer || 0 == gFwBufferSize)
    {
        NXPLOG_FWDNLD_D("Firmware buffer and size must be set before calling download");
        return NFCSTATUS_FAILED;
    }

    /* load the address of download image pointer and image size */
    // gpFwBuffer and gFwBufferSize must be set at the top layer before the download
    // is started.
    *pImgInfo  = gpFwBuffer;
    *pImgInfoLen = gFwBufferSize;
