...
int listInit(struct listHead* pList)
{
    ...
    pList->pFirst = NULL;
    if ( (status = phOsalNfc_CreateMutex(&pList->mutex)) != NFCSTATUS_SUCCESS)
    {
        NXPLOG_NCIHAL_E("Mutex creation failed (status=0x%08x)", status);
        return 0;
    }
...
int listDestroy(struct listHead* pList)
{
    ...
    if ( (status = phOsalNfc_DeleteMutex(&pList->mutex)) != NFCSTATUS_SUCCESS)
    {
        NXPLOG_NCIHAL_E("Mutex destruction failed (status=0x%08x)", status);
        return 0;
    }
...
int listAdd(struct listHead* pList, void* pData)
{
    ...
    /* Create node */
    pNode = (struct listNode*) phOsalNfc_GetMemory(sizeof(struct listNode));
    if (pNode == NULL)
    {
        result = 0;
    ...
    phOsalNfc_LockMutex(pList->mutex);
...
clean_and_return:
    phOsalNfc_UnlockMutex(pList->mutex);
    return result;
}
...
int listRemove(struct listHead* pList, void* pData)
{
    ...
    /* Deallocate the node */
    phOsalNfc_FreeMemory(pRemovedNode);
...
phNxpNciHal_init_monitor(void)
{
    ...
    {
        nxpncihal_monitor = (phNxpNciHal_Monitor_t *) phOsalNfc_GetMemory(
                sizeof(phNxpNciHal_Monitor_t));
    }

    if (nxpncihal_monitor != NULL)
    {
    	phOsalNfc_SetMemory(nxpncihal_monitor, 0x00, sizeof(phNxpNciHal_Monitor_t));

        wStatus = phOsalNfc_CreateMutex(&nxpncihal_monitor->reentrance_mutex);
        if (wStatus != NFCSTATUS_SUCCESS)
        {
            NXPLOG_NCIHAL_E("reentrance_mutex creation returned error: %u", wStatus);
            goto clean_and_return;
        }

        wStatus = phOsalNfc_CreateMutex(&nxpncihal_monitor->concurrency_mutex);
        if (wStatus != NFCSTATUS_SUCCESS)
        {
            NXPLOG_NCIHAL_E("concurrency_mutex creation returned error: %u", wStatus);
            phOsalNfc_DeleteMutex(&nxpncihal_monitor->reentrance_mutex);
            goto clean_and_return;
        }
        ...
        if (listInit(&nxpncihal_monitor->sem_list) != 1)
        {
            ...
            phOsalNfc_DeleteMutex(&nxpncihal_monitor->concurrency_mutex);
            phOsalNfc_DeleteMutex(&nxpncihal_monitor->reentrance_mutex);
            goto clean_and_return;
        }
    }
...
clean_and_return:
    ...
    if (nxpncihal_monitor != NULL)
    {
    	phOsalNfc_FreeMemory(nxpncihal_monitor);
        ...
        void *pContext)
{
    ...
    /* Create semaphore */
    if ( (status = phOsalNfc_CreateSemaphore(&pCallbackData->sem, 0)) != NFCSTATUS_SUCCESS)
    {
        NXPLOG_NCIHAL_E("Semaphore creation failed (status=0x%08x)", status);
...
void phNxpNciHal_cleanup_cb_data(phNxpNciHal_Sem_t* pCallbackData)
{
	NFCSTATUS status;
    /* Destroy semaphore */
    if ( (status = phOsalNfc_DeleteSemaphore(&pCallbackData->sem)) != NFCSTATUS_SUCCESS)
    {
        NXPLOG_NCIHAL_E("phNxpNciHal_cleanup_cb_data: Failed to destroy semaphore (status=0x%08x)", status);
    }
...
void phNxpNciHal_releaseall_cb_data(void)
{
    ...
    {
        pCallbackData->status = NFCSTATUS_FAILED;
        phOsalNfc_ProduceSemaphore(pCallbackData->sem);
    }
        ...
        uint16_t len)
{
    ...
    char* print_buffer = (char*)ry_malloc(len*3+1);
	if (print_buffer == NULL) {
	    return;
	}
    
    //phOsalNfc_SetMemory (print_buffer, 0, sizeof(print_buffer));
    phOsalNfc_SetMemory (print_buffer, 0, len*3+1);
    for (i = 0; i < len; i++) {
