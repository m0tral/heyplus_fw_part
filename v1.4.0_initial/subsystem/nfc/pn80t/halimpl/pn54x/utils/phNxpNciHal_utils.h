...
#define SEM_WAIT(cb_data) phOsalNfc_ConsumeSemaphore(((cb_data).sem))
#define SEM_POST(p_cb_data) phOsalNfc_ProduceSemaphore(((p_cb_data)->sem))

/* Semaphore and mutex monitor */
typedef struct phNxpNciHal_Monitor
{
    /* Mutex protecting native library against reentrance */
	void* reentrance_mutex;

    /* Mutex protecting native library against concurrency */
	void* concurrency_mutex;
...
#define REENTRANCE_LOCK()        phOsalNfc_LockMutex(phNxpNciHal_get_monitor()->reentrance_mutex)
#define REENTRANCE_UNLOCK()      phOsalNfc_UnlockMutex(phNxpNciHal_get_monitor()->reentrance_mutex)
#define CONCURRENCY_LOCK()       phOsalNfc_LockMutex(phNxpNciHal_get_monitor()->concurrency_mutex)
#define CONCURRENCY_UNLOCK()     phOsalNfc_UnlockMutex(phNxpNciHal_get_monitor()->concurrency_mutex)
