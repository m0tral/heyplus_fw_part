...
{ 
  ...
  mible_onConnect(pMsg->connOpen.hdr.param, pMsg->connOpen.peerAddr, 
    pMsg->connOpen.addrType, pMsg->connOpen.role);

  // ble_updateConnPara(6, 12, 600, 0);

  LOG_DEBUG("[appSlaveProcConnOpen] connId: 0x%04x\n", pMsg->connOpen.hdr.param);
  LOG_DEBUG("[appSlaveProcConnOpen] addrType: 0x%04x\n", pMsg->connOpen.addrType);
  LOG_DEBUG("[appSlaveProcConnOpen] peerAddr: %02x:%02x:%02x:%02x:%02x:%02x\n\n", 
      pMsg->connOpen.peerAddr[0], pMsg->connOpen.peerAddr[1], pMsg->connOpen.peerAddr[2],
      pMsg->connOpen.peerAddr[3], pMsg->connOpen.peerAddr[4], pMsg->connOpen.peerAddr[5]);
...
static void appSlaveProcConnClose(dmEvt_t *pMsg, appConnCb_t *pCb)
{
  ...
  LOG_DEBUG("[appSlaveProcConnClose] Disconnect. ConnId: 0x%04x, Reason:0x%02x\n", 
      pMsg->connClose.handle, pMsg->connClose.reason);
  mible_onDisconnect(pMsg->connClose.handle);
...
void AppSlaveProcDmMsg(dmEvt_t *pMsg)
{
  ...
  switch (pMsg->hdr.event)
  {
    ...
    case DM_HW_ERROR_IND:
      LOG_ERROR("[BLE_HCI] DM_HW_ERROR_IND\n");
      HciDrvRadioBoot(0);
      DmDevReset();
      break;
                 ...
                 uint8_t *pMaxEaEvents, bool_t cfgAdvParam)
{
  ...
  if (!appSlaveConnectableAdv(numSets, pAdvHandles) || (appNumConns(DM_ROLE_SLAVE) < pAppSlaveCfg->connMax))
  {
    ...
    for (i = 0; i < numSets; i++)
    {
      ...
      {
        /* set adv channel map */
        DmAdvSetChannelMap(pAdvHandles[i], DM_ADV_CHAN_ALL); // For debug
                      ...
                      uint8_t *pMaxEaEvents, bool_t cfgAdvParam, uint8_t mode)
{
  ...
  if (mode == APP_MODE_AUTO_INIT)
  {
    if(0)   //set white list
    {
        if (AppDbCheckBonded() == TRUE)
        {
            void* p_current_record = AppDbGetNextRecord(APP_DB_HDL_NONE);
            while(p_current_record != APP_DB_HDL_NONE)
            {
                if(AppDbRecordInUse(p_current_record))
                {
                    bdAddr_t addr;
                    uint8_t addr_type;
                    AppDbGetWhiteListResourceFromRecord(p_current_record, addr, &addr_type);

                    DmDevWhiteListClear();
                    DmDevWhiteListAdd(addr_type, addr);
    //                BdaCpy(appSlaveCb.peerAddr, addr);
    //                appSlaveCb.peerAddrType = addr_type;
                    break;
                }
            }
        }
...
void AppSlaveSecProcDmMsg(dmEvt_t *pMsg)
{
  ...
  switch(pMsg->hdr.event)
  {
    ...
    case DM_SEC_PAIR_CMPL_IND:
      ...
      LOG_INFO("AppSlaveSecProcDmMsg => DM_SEC_PAIR_CMPL_IND");
      break;
      
    case DM_SEC_PAIR_FAIL_IND:
      appSecPairFailed(pMsg, pCb);
      LOG_INFO("AppSlaveSecProcDmMsg => DM_SEC_PAIR_FAIL_IND");
      break;

    case DM_SEC_ENCRYPT_IND:
      appSecEncryptInd(pMsg, pCb);
      LOG_INFO("AppSlaveSecProcDmMsg => DM_SEC_ENCRYPT_IND");
      break;
    ...
    case DM_SEC_KEY_IND:
      ...
      LOG_INFO("AppSlaveSecProcDmMsg => DM_SEC_KEY_IND");
      break;

    case DM_SEC_PAIR_IND:
      appSecPairInd(pMsg, pCb);
      LOG_INFO("AppSlaveSecProcDmMsg => DM_SEC_PAIR_IND");
      break;
      
    case DM_SEC_LTK_REQ_IND:
      appSecFindLtk(pMsg, pCb);
      LOG_INFO("AppSlaveSecProcDmMsg => DM_SEC_LTK_REQ_IND");
      break;
