...
static void appDiscStart(dmConnId_t connId)
{
  ...
  {
    hdl = AppDbGetHdl(connId);
    /* get discovery status */
    if (hdl != APP_DB_HDL_NONE)
    {
      status = AppDbGetDiscStatus(hdl);
      LOG_DEBUG("[app_disc] LOAD from db and status:%d\n", status);
    }
    else
    {
      status = pAppDiscCb->cmplStatus;
      LOG_DEBUG("[app_disc] Rest DB not Found\n");
    }
...
void AppDiscProcAttMsg(attEvt_t *pMsg)
{
  ...
  if (pAppDiscCb->inProgress == APP_DISC_IN_PROGRESS)
  {
    ...
    if (pMsg->hdr.event == ATTC_FIND_BY_TYPE_VALUE_RSP)
    {
      ...
      LOG_DEBUG("[app_disc]AttcDiscServiceCmpl1 status 0x%02x\n", status);
             ...
             pMsg->hdr.event == ATTC_FIND_INFO_RSP)
    {
      ...
      LOG_DEBUG("[app_disc]AttcDiscCharCmpl2 status 0x%02x\n", status);
           ...
           (pMsg->hdr.event == ATTC_READ_RSP || pMsg->hdr.event == ATTC_WRITE_RSP))
  {
    ...
    else
    {
      ...
      LOG_DEBUG("[app_disc]AttcDiscConfigCmpl3 status 0x%02x\n", status);
...
void AppDiscComplete(dmConnId_t connId, uint8_t status)
{
  ...
  if (hdl != APP_DB_HDL_NONE)
  {
    ...
    if (!(status == APP_DISC_CFG_CMPL && pAppDiscCb->connCfgStatus == APP_DISC_CFG_CONN_START))
    {
      uint8_t current_status = AppDbGetDiscStatus(hdl);
      if((current_status != APP_DISC_CFG_CMPL)
        &&(status == APP_DISC_CFG_CMPL))
      {
        LOG_DEBUG("[app_disc] will not store the handle and status\n");
//        LOG_DEBUG("[app_disc]status changed\n");
//        isSaveRequired = true;
      }
    ...
    if (pAppDiscCb->pHdlList != NULL)
    {
      ...
      if((status == APP_DISC_CMPL)
        &&
        (memcmp(pAppDiscCb->pHdlList, AppDbGetHdlList(hdl), (pAppDiscCb->hdlListLen * sizeof(uint16_t))) != 0)
        )
      {
        LOG_DEBUG("[app_disc] APP_DISC_CMPL store handle\n");
        AppDbSetHdlList(hdl, pAppDiscCb->pHdlList);
        isSaveRequired = true;
      }
    }
  ...
  LOG_DEBUG("[app_disc]AppDiscComplete connId:%d status:0x%02x\n", connId, status);
}
