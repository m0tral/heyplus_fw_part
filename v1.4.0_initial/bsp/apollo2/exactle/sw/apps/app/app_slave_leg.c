...
void AppAdvStart(uint8_t mode)
{
  ...
  if (appSlaveAdvMode())
  {
    advHandle = DM_ADV_HANDLE_DEFAULT;
    maxEaEvents = 0;
    appSlaveCb.advType[DM_ADV_HANDLE_DEFAULT] = DM_ADV_CONN_UNDIRECT;

    /* initialize advertising state */
    //if(is_factory_test_finish() || (dev_mfg_state_get() < DEV_MFG_STATE_ACTIVATE)){

      appSlaveCb.advState[DM_ADV_HANDLE_DEFAULT] = APP_ADV_STATE2;

      appSlaveAdvStart(1, &advHandle, &(pAppAdvCfg->advInterval[APP_ADV_STATE2]),
                     &(pAppAdvCfg->advDuration[APP_ADV_STATE2]), &maxEaEvents, TRUE, mode);
    //} else {
    //  appSlaveCb.advState[DM_ADV_HANDLE_DEFAULT] = APP_ADV_STATE2;
...
void RyeexUnbindAdvStart(uint8_t mode)
{
  ...
  if (appSlaveAdvMode())
  {
    ...
    appSlaveCb.advState[DM_ADV_HANDLE_DEFAULT] = APP_ADV_STATE4;
    appSlaveCb.advType[DM_ADV_HANDLE_DEFAULT] = DM_ADV_CONN_UNDIRECT;

    appSlaveAdvStart(1, &advHandle, &(pAppAdvCfg->advInterval[APP_ADV_STATE4]),
                 &(pAppAdvCfg->advDuration[APP_ADV_STATE4]), &maxEaEvents, TRUE, mode);
...
void RyeexUnconnectableAdvStart(uint8_t mode)
{
  ...
  if (appSlaveAdvMode())
  {
    ...
    appSlaveCb.advState[DM_ADV_HANDLE_DEFAULT] = APP_ADV_STATE3;
    appSlaveCb.advType[DM_ADV_HANDLE_DEFAULT] = DM_ADV_CONN_UNDIRECT;

    appSlaveAdvStart(1, &advHandle, &(pAppAdvCfg->advInterval[APP_ADV_STATE3]),
                 &(pAppAdvCfg->advDuration[APP_ADV_STATE3]), &maxEaEvents, TRUE, mode);
