...
void hciEvtProcessCmdCmpl(uint8_t *p, uint8_t len)
{
  ...
  default:
    ...
    if (HCI_OGF(opcode) == HCI_OGF_VENDOR_SPEC)
    {
      ...
      if(HCI_OCF(opcode) == 0x20)
      {
        // ReadAtAddress returns
        hciEvt_t Temp;
        //       
        hciEvtParseVendorSpecCmdCmpl(&Temp, p, len);
        p -= 2;

        BSTREAM_TO_UINT16(Temp.vendorSpecCmdCmpl.opcode, p);
        BSTREAM_TO_UINT8(Temp.hdr.status, p);
        BSTREAM_TO_UINT32(xtrim_regValue, p);

        if(Temp.hdr.status == 0)
        {
            uint8_t *p = Temp.vendorSpecCmdCmpl.param;
            // BSTREAM_TO_UINT32(xtrim_regValue, p);     
            LOG_DEBUG("register read value = 0x%08x \n", xtrim_regValue);
        }
        else
        {
            LOG_DEBUG("VS command returned error = 0x%2x \n", Temp.hdr.status);
        }
      }
    }
    break;
