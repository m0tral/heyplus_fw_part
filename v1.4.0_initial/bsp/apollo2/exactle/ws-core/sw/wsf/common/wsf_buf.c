...
#if !defined(WSF_USE_RY_MALLOC)
#define WSF_USE_RY_MALLOC 1
#endif
...
{
    void* p_buff = (void*)ry_malloc(len);
#if WSF_OS_DIAG == TRUE
...
//        LOG_DEBUG("[wsf_malloc] alloc:%d\n", len);
  ...
  {
    LOG_ERROR("[wsf_malloc] alloc failed:%d\n", len);
  }
#else
  LOG_ERROR("[wsf_malloc] alloc failed:%d\n", len);
#endif
