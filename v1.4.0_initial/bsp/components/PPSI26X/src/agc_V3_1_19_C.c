...
//#define NRF_LOG_MODULE_NAME "agc"
//#include "nrf_log.h"
...
#define SATURATELOW ((uint16_t) 3000)
#define SATURATEHIGH ((uint16_t) 62000)
...
uint8_t OFFOPTIONSTIMES=2;


uint16_t AMPOPTIONS[AMPOPTIONSLENGTH] = {10,25,50,100,250,500,1000,2000};
//uint16_t AMPOPTIONS[AMPOPTIONSLENGTH] = {10,25,50,100,250,500,1000,1500,2000};
...
int16_t AGC_InitOff1 = -1000;
uint8_t AGC_InitOffPos1 = 40;
int16_t AGC_InitOff2 = -25;
uint8_t AGC_InitOffPos2 = 1;
int16_t AGC_InitOff3 = -1000;
uint8_t AGC_InitOffPos3 = 40;
int16_t AGC_InitOff4 = -1000;
uint8_t AGC_InitOffPos4 = 40;

uint8_t AGC_InitLEDCur1 = 64;
uint8_t AGC_InitLEDCur2 = 64;
uint8_t AGC_InitLEDCur3 = 64;
...
void AGC_InitializeAGC(void)
{
    ...
    AFE4410_setLedCurrent_P(1,255); //178->70mA// Green 0-255-->0-100mA
    AFE4410_setLedCurrent_P(2,25); //19*2);  // IR 0-255-->0-100mA
    AFE4410_setLedCurrent_P(3,0);  // Red 0-255-->0-100mA

    AFE4410_setCf_ENSEPGAIN(1,2);// 2:100
    AFE4410_setCf_ENSEPGAIN(2,2);// 2:100

    AFE4410_setRf_ENSEPGAIN(1,500); //green/amb
    AFE4410_setRf_ENSEPGAIN(2,500); //ir/red

//    AFE4410_SetOffsetDAC(1,-900);
//    AFE4410_SetOffsetDAC(2,-25);
//    AFE4410_SetOffsetDAC(3,-600);
//    AFE4410_SetOffsetDAC(4,-600);
...
agc_state_t AGC_ServiceCL(uint16_t Ch1Code, uint16_t Ch2Code, uint16_t Ch3Code, uint16_t Ch4Code, bool AdjustmentMade)
{
    AGCStruct.Counter_SamplesSinceStateChange = AGCStruct.Counter_SamplesSinceStateChange + 1;
    // AGC State Machine
    switch (AGCStruct.AGCCont_AGCState) {
        case AGC_STATE_WAITINGFORADJUSTMENT:
            //NRF_LOG_RAW_INFO("AGC_STATE_WAITINGFORADJUSTMENT\r\n");
            // Waits until adjustment has been made or TIMEOUT occurs
            if(AdjustmentMade) {
                AGCStruct.AGCCont_AGCState = AGC_STATE_INITIALIZEFILTER;
                AGCStruct.Counter_SamplesSinceStateChange = 0;
        ...
        case AGC_STATE_ERRORSTATE:
            ...
            //NRF_LOG_RAW_INFO("AGC_STATE_ERRORSTATE\r\n");
            // Realigns AGC with AFE before attempting to resume with calculations
            if(AGCStruct.Counter_SamplesSinceStateChange>0) {
                AlignAGCWithAFE();
                AGCStruct.AGCCont_AGCState = AGC_STATE_INITIALIZEFILTER;
...
//
        case AGC_STATE_INITIALIZEFILTER:
            ...
            //NRF_LOG_RAW_INFO("AGC_STATE_INITIALIZEFILTER\r\n");
            // Ensure unexpected adjustment did not occur
            if(!AdjustmentMade) {
                // If stabilization time not reached, update signal states, check saturation and adjust, otherwise initialise filter
                if (AGCStruct.Counter_SamplesSinceStateChange <= FILTERSTABILIZATIONTIME) {
        ...
        case AGC_STATE_FILTERSTABLE:
            ...
            //NRF_LOG_RAW_INFO("AGC_STATE_FILTERSTABLE\r\n");
            // Ensure unexpected adjustment did not occur
            if(!AdjustmentMade) {
                //Update various signal states and checks
                UpdateSignalState(Ch1Code,Ch2Code,Ch3Code,Ch4Code);
    ...
    switch (AGCStruct.AGCCont_Calib_State) {
            ...
            for (counter = 0; counter < 3; counter ++) {
                if(AGCStruct.AGCCont_LEDCur[counter]) {
                    if(ChannelTooHigh(channelInd[counter])) {
                        AGCStruct.Settings_LEDCur[counter] = 64;//16;
                        AGCStruct.Adj_LEDCur[counter] = TRUE;
                        retVal = TRUE;
                    }
                    if(ChannelTooLow(channelInd[counter])) {
                        AGCStruct.Settings_LEDCur[counter] = 193;//48;
                        AGCStruct.Adj_LEDCur[counter] = TRUE;
                        retVal = TRUE;
                    }
                }
        ...
        case CALIB_STATE_BIN_SEARCH_1:
            for (counter = 0; counter < 3; counter ++) {
                if(AGCStruct.AGCCont_LEDCur[counter]) {
                    if(ChannelTooLow(channelInd[counter])) {
                        if(AGCStruct.Settings_LEDCur[counter] == 64) { //16
                            AGCStruct.Settings_LEDCur[counter] = 96;//24
                            AGCStruct.Adj_LEDCur[counter] = TRUE;
                            retVal = TRUE;
                        } else if(AGCStruct.Settings_LEDCur[counter] == 129) { //32
                            AGCStruct.Settings_LEDCur[counter] = 161;//40;
                            AGCStruct.Adj_LEDCur[counter] = TRUE;
                            retVal = TRUE;
                        } else if(AGCStruct.Settings_LEDCur[counter] == 193) { //48
                            AGCStruct.Settings_LEDCur[counter] = 225;//56;
                            AGCStruct.Adj_LEDCur[counter] = TRUE;
                            retVal = TRUE;
                        }
                    }
                    if(ChannelTooHigh(channelInd[counter])) {
                        if(AGCStruct.Settings_LEDCur[counter] == 64) { //16)
                            AGCStruct.Settings_LEDCur[counter] = 32;//8;
                            AGCStruct.Adj_LEDCur[counter] = TRUE;
                            retVal = TRUE;
                        } else if(AGCStruct.Settings_LEDCur[counter] == 129) { //32
                            AGCStruct.Settings_LEDCur[counter] = 96;//24;
        ...
        case CALIB_STATE_SELECT_LED_CURRENT:
            for (counter = 0; counter < 3; counter++) {
                if(AGCStruct.AGCCont_LEDCur[counter]) {
                    ...
                    if(AGCStruct.Settings_LEDCur[counter] != (uint8_t) AGC_min(255,(uint16_t)(((float)(6000000))/CTR[counter]))) {
                        AGCStruct.Settings_LEDCur[counter] = (uint8_t) AGC_min(255,(uint16_t)(((float)(6000000))/CTR[counter]));
                        AGCStruct.Adj_LEDCur[counter] = TRUE;
                        retVal = TRUE;
                    }
                }
...
bool CheckOffAdjustement(void)
{
    if(AGCStruct.AGCCont_Off) {
        ...
        //1uA:400 0000/Rf/2/1000 000

        //AGCStruct.SigSum_FiltValDC[0] - 0x7fff -->ADC'S DC Value(400 0000)
        //(tmpVal[0]*75 - AGCStruct.Settings_Off[0]*100000) * 2^10
#if 0
        tmpVal[0] = (tmpConst1 * (AGCStruct.SigSum_FiltValDC[0] - 0x7fff))>>10;
        tmpVal[0] = tmpVal[0]*75 - AGCStruct.Settings_Off[0]*100000;//-->A (安培，电流)
        ...
        tmpVal[0] = (tmpConst1 * (AGCStruct.SigSum_FiltValDC[0] - 0x7fff))>>10;
        tmpVal[0] = (tmpVal[0]*1575/10 - AGCStruct.Settings_Off[0]*10000)*OFFOPTIONSTIMES;//-->A (安培，电流)
        tmpVal[1] = (tmpConst1 * (AGCStruct.SigSum_FiltValDC[1] - 0x7fff))>>10;
        tmpVal[1] = (tmpVal[1]*1575/10 - AGCStruct.Settings_Off[1]*10000)*OFFOPTIONSTIMES;
        tmpVal[2] = (tmpConst2 * (AGCStruct.SigSum_FiltValDC[2] - 0x7fff))>>10;
        tmpVal[2] = (tmpVal[2]*1575/10 - AGCStruct.Settings_Off[2]*10000)*OFFOPTIONSTIMES;
        tmpVal[3] = (tmpConst2 * (AGCStruct.SigSum_FiltValDC[3] - 0x7fff))>>10;
        tmpVal[3] = (tmpVal[3]*1575/10 - AGCStruct.Settings_Off[3]*10000)*OFFOPTIONSTIMES;
        ...
        for (counter1 = 0; counter1 < 4; counter1++) {
            ...
            for (counter2 = 0; counter2 < OFFOPTIONSLENGTH; counter2++) {
                ...
                if(counter2 != 0) {
                    LowerBound = -10000*(1*((int32_t)(OFFOPTIONS[counter2-1])+(int32_t)(OFFOPTIONS[counter2]))*OFFOPTIONSTIMES/2);
                } else {
                    LowerBound = -2147483647;
                }
                if(counter2 != OFFOPTIONSLENGTH-1) {
                    HigherBound = -10000*(1*((int32_t)(OFFOPTIONS[counter2])+(int32_t)(OFFOPTIONS[counter2+1]))*OFFOPTIONSTIMES/2);
                } else {
...
bool CheckLEDCurAdjustement(void)
{
    if(AGCStruct.AGCCont_LEDCur[0] || AGCStruct.AGCCont_LEDCur[1] || AGCStruct.AGCCont_LEDCur[2]) {
        ...
        tmpVal[0] = (tmpConst1 * (AGCStruct.SigSum_FiltValDC[0] - 0x7fff))>>10;
        tmpVal[0] = (tmpVal[0]*1575/10 - AGCStruct.Settings_Off[0]*10000)*OFFOPTIONSTIMES;
        tmpVal[1] = (tmpConst1 * (AGCStruct.SigSum_FiltValDC[1] - 0x7fff))>>10;
        tmpVal[1] = (tmpVal[1]*1575/10 - AGCStruct.Settings_Off[1]*10000)*OFFOPTIONSTIMES;
        tmpVal[2] = (tmpConst2 * (AGCStruct.SigSum_FiltValDC[2] - 0x7fff))>>10;
        tmpVal[2] = (tmpVal[2]*1575/10 - AGCStruct.Settings_Off[2]*10000)*OFFOPTIONSTIMES;
        ...
        if(AGCStruct.AGCCont_LEDCur[0]) {
            ...
            if(((tmpVal[0]- tmpVal[1]) > OBJMAXVALLEDCONT) && (AGCStruct.Settings_LEDCur[0] > 3)) {
                ...
                AGCStruct.Adj_LEDCur[0] = TRUE;
            } else if(((tmpVal[0] - tmpVal[1]) < OBJMINVALLEDCONT) && (AGCStruct.Settings_LEDCur[0] < 254)) {
        ...
        if(AGCStruct.AGCCont_LEDCur[1]) {
            ...
            if(((tmpVal[2]- tmpVal[1]) > OBJMAXVALLEDCONT) && (AGCStruct.Settings_LEDCur[1] > 3)) {
                //Decrease channel x setting
                AGCStruct.Settings_LEDCur[1] = AGCStruct.Settings_LEDCur[1] - 1; //cole modify fixed ir led current
                AGCStruct.Adj_LEDCur[1] = TRUE;
            } else if(((tmpVal[2] - tmpVal[1]) < OBJMINVALLEDCONT) && (AGCStruct.Settings_LEDCur[1] < 254)) {
                //Increase channel x setting
                AGCStruct.Settings_LEDCur[1] = AGCStruct.Settings_LEDCur[1] + 1;//cole modify fixed ir led current
                AGCStruct.Adj_LEDCur[1] = TRUE;
            }
        ...
        if(AGCStruct.AGCCont_LEDCur[2]) {
            ...
            if(((tmpVal[3]- tmpVal[1]) > OBJMAXVALLEDCONT) && (AGCStruct.Settings_LEDCur[2] > 3)) {
                //Decrease channel x setting
                AGCStruct.Settings_LEDCur[2] = AGCStruct.Settings_LEDCur[2] - 1;//cole modify fixed red led current
                AGCStruct.Adj_LEDCur[2] = TRUE;
            } else if(((tmpVal[3] - tmpVal[1]) < OBJMINVALLEDCONT) && (AGCStruct.Settings_LEDCur[2] < 254)) {
                //Increase channel x setting
                AGCStruct.Settings_LEDCur[2] = AGCStruct.Settings_LEDCur[2] + 1;//cole modify fixed red led current
                AGCStruct.Adj_LEDCur[2] = TRUE;
            }
...
bool CheckChannelSaturationAdjustment(uint8_t ChannelNumber)
{
    ...
    if(ChannelState == CHANNEL_STATE_SATURATEHIGH) {
        ...
        if(AGCStruct.AGCCont_Off) {
            ...
            if(AGCStruct.Settings_Off_Pos[ChannelNumber] < OFFOPTIONSLENGTH-1) {
                AGCStruct.Settings_Off_Pos[ChannelNumber] = AGCStruct.Settings_Off_Pos[ChannelNumber]+3;//1-->3
        ...
        if(ChannelNumber != 1 && (!(retVal))) {
            if(AGCStruct.AGCCont_LEDCur[LEDInd[ChannelNumber]] && AGCStruct.Settings_LEDCur[LEDInd[ChannelNumber]] > 10) {
        ...
        if(AGCStruct.AGCCont_Off) {
            if(AGCStruct.Settings_Off_Pos[ChannelNumber] > 0) {
                AGCStruct.Settings_Off_Pos[ChannelNumber] = AGCStruct.Settings_Off_Pos[ChannelNumber]-2;//1-->2
                AGCStruct.Settings_Off[ChannelNumber] = OFFOPTIONS[AGCStruct.Settings_Off_Pos[ChannelNumber]];
                AGCStruct.Adj_Off[ChannelNumber] = TRUE;
                retVal = TRUE;
            }
        ...
        if(ChannelNumber != 1 && (!(retVal))) {
            if(AGCStruct.AGCCont_LEDCur[LEDInd[ChannelNumber]] && AGCStruct.Settings_LEDCur[LEDInd[ChannelNumber]] < 250) {
                AGCStruct.Settings_LEDCur[LEDInd[ChannelNumber]] = AGCStruct.Settings_LEDCur[LEDInd[ChannelNumber]] + 1;
                AGCStruct.Adj_LEDCur[LEDInd[ChannelNumber]] = TRUE;
                retVal = TRUE;
            }
    ...
    //NRF_LOG_RAW_INFO("CheckChannelSaturationAdjustment,%d %d %d\r\n",ChannelNumber,(uint32_t)(AGCStruct.AGCCont_Off),retVal);
...
bool SignalSaturated(void)
{
    ...
    //NRF_LOG_RAW_INFO("SignalSaturated,%d %d %d %d\r\n",retVal1,retVal2,retVal3,retVal4);
...
void InitializeFilter(uint16_t Ch1Code, uint16_t Ch2Code, uint16_t Ch3Code, uint16_t Ch4Code)
{
    ...
    //NRF_LOG_RAW_INFO("InitializeFilter\r\n");
}
...
void AdjustAFE(void)
{
    ...
    for (counter = 0; counter < 2; counter++) {
        if (AGCStruct.Adj_Amp[counter]) {
            ...
            AFE4410_setCf_ENSEPGAIN(counter+1,AGCStruct.Settings_Amp[counter]);
        }
    }
    for (counter = 0; counter < 4; counter++) {
        if (AGCStruct.Adj_Off[counter]) {
            //AFE4404_setAmbientCurrent(counter+1,AGCStruct.Settings_Off[counter]);
            AFE4410_SetOffsetDAC (counter+1 , AGCStruct.Settings_Off[counter]);
        }

        //NRF_LOG_RAW_INFO("AdjustAFE:counter,Adj_Off,Settings_Off %d %d %d\r\n",counter,AGCStruct.Adj_Off[counter],AGCStruct.Settings_Off[counter]);
    }
    for (counter = 0; counter < 3; counter++) {
        if (AGCStruct.Adj_LEDCur[counter]) {
            //AFE4404_setLedCurrent(counter+1,AGCStruct.Settings_LEDCur[counter]);
            AFE4410_setLedCurrent_P(counter+1 ,AGCStruct.Settings_LEDCur[counter]);
        }
    }
    ResetAGCStruct();
...
void AlignAGCWithAFE(void)
{
    ...
    if(AGCStruct.Settings_currentAFEsettings != 0) {
        AGCStruct.Settings_Off[0] = AFE4410_settingsUintGetIsub(1,AGCStruct.Settings_currentAFEsettings);
        AGCStruct.Settings_Off[1] = AFE4410_settingsUintGetIsub(2,AGCStruct.Settings_currentAFEsettings);
        AGCStruct.Settings_Off[2] = AFE4410_settingsUintGetIsub(3,AGCStruct.Settings_currentAFEsettings);
        AGCStruct.Settings_Off[3] = AFE4410_settingsUintGetIsub(4,AGCStruct.Settings_currentAFEsettings);
        AGCStruct.Settings_Amp[0] = AFE4410_settingsUintGetRf(1,AGCStruct.Settings_currentAFEsettings);
        AGCStruct.Settings_Amp[1] = AFE4410_settingsUintGetRf(2,AGCStruct.Settings_currentAFEsettings);
        AGCStruct.Settings_LEDCur[0] = AFE4410_settingsUintGetLedCurrent(1,AGCStruct.Settings_currentAFEsettings);
        AGCStruct.Settings_LEDCur[1] = AFE4410_settingsUintGetLedCurrent(2,AGCStruct.Settings_currentAFEsettings);
        AGCStruct.Settings_LEDCur[2] = AFE4410_settingsUintGetLedCurrent(3,AGCStruct.Settings_currentAFEsettings);
    } else {

        AGCStruct.Settings_Off[0] = AFE4410_GetOffsetDAC(1);
        AGCStruct.Settings_Off[1] = AFE4410_GetOffsetDAC(2);
        AGCStruct.Settings_Off[2] = AFE4410_GetOffsetDAC(3);
        AGCStruct.Settings_Off[3] = AFE4410_GetOffsetDAC(4);
        AGCStruct.Settings_Amp[0] = AFE4410_GetRF(1);
        AGCStruct.Settings_Amp[1] = AFE4410_GetRF(2);
        AGCStruct.Settings_LEDCur[0] = AFE4410_getLedCurrent_P(1);
        AGCStruct.Settings_LEDCur[1] = AFE4410_getLedCurrent_P(2);
        AGCStruct.Settings_LEDCur[2] = AFE4410_getLedCurrent_P(3);
    }


    //NRF_LOG_RAW_INFO("AlignAGCWithAFE\r\n");
}
