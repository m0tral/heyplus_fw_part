	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				...
				duration = BMA4_SET_BITSLICE(duration, BMA456_ANY_NO_MOTION_DUR, any_motion->duration);
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				...
				16 bits of data */
				data = BMA4_SET_BITSLICE(data, BMA456_STEP_CNTR_WM,
					step_counter_wm);
	...
	if (dev != NULL) {
		if (dev->chip_id == BMA456_CHIP_ID) {
			...
			if (rslt == BMA4_OK) {
				...
				wm_msb = feature_config[index + 1] << 8;
				data = wm_lsb | wm_msb;
				*step_counter_wm = BMA4_GET_BITSLICE(data, BMA456_STEP_CNTR_WM);
			}
		} else {
			rslt = BMA4_E_INVALID_SENSOR;
		}
