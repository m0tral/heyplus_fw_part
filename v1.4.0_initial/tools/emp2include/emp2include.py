	...
	for subdir, dirs, files in os.walk('./'):
		for file in files:
			if file.endswith('.emp'):
				if (subdir.endswith('iram_development') and (memory == 'IRAM')):
					print('./iram_development/'+ file)
					asps.append('./iram_development/'+ file)

				if (subdir.endswith('otp_production') and (memory == 'OTP')):
					print('./otp_production/'+ file)
					asps.append('./otp_production/'+ file)
	...
	for file in asps:
		...
		while ((cmdIndex < len(binarray)) & ((len(binarray) - cmdIndex) > 20)):
			...
			if(binarray[cmdIndex:cmdIndex+4] == b'39me'):
				...
				S += '0x{:04X}'.format(binarray[cmdIndex+12] + (binarray[cmdIndex+13] << 8))
				S += ', '

				# Add the hex representation of the patch User Build Number
				S += '0x{:04X}'.format(binarray[cmdIndex+14] + (binarray[cmdIndex+15] << 8))
				S += ', '

				# Add the hex representation of the patch Format Version
				S += '0x{:02X}'.format(binarray[cmdIndex+8])
				S += ', '

				# Add the hex representation of the patch Container Type
				S += '0x{:02X}'.format(binarray[cmdIndex+9])
				S += ', '
				...
				S += '0x{:02X}'.format(binarray[cmdIndex+10])
				S += ', '
